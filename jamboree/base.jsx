...
window.CCScope = React.createContext('');                // 덱 인스턴스 접두사 → 같은 템플릿 여러 장을 독립 편집
...
function Placeholder({ label = '현장 사진', tone = 'light', radius = 0, slot, slotLabel, style, bare }) {
  ...
  const sc = React.useContext(window.CCScope);
  const sslot = slot ? sc + slot : slot;   // 덱 인스턴스 접두사
  React.useEffect(() => { if (slot && register) register(sslot, slotLabel || label || slot); }, [sslot]);
