...
- 📅 **예약된 작업 — 2026년 말 KV 일괄 삭제**(사용자 확정 2026-08-14). 백업은 **이미 끝났다**(외장 `Slave_Stick` 의 `Legacy/`, 키 66/66 · 사진 29/29 · 개인정보 CSV 2종 — 정확한 경로는 아래 📍). 실행은 `node scripts/purge-retired-kv.js --confirm` — 기본은 미리보기다.
  - ⚠️ **접두사로 지우면 운영 중인 보드가 날아간다**: `jp:` 73개는 홍보부 것이고 `jp:fnc*` 4개만 급식본부 것 · `img:` 118개 중 **89개가 제보·홍보부 사진**이다. 스크립트가 허용 목록 + 백업 대조로 막고 있으니 **직접 `wrangler kv key delete` 를 치지 말 것.**
  - ⚠️ 지운 뒤 `/service-ended?s=dcount` 의 *"사진은 모두 안전하게 보관하고 있습니다"* 문구가 **거짓이 된다** → 그때 문구 수정 여부를 사용자에게 확인.
  - `hit:/krjam-*` 21개는 90일 TTL 로 자동 소멸 — 손대지 않는다.
  - 📍 **백업은 바탕화면이 아니라 외장디스크에 있다**(2026-08-14 사용자가 이관): `/Volumes/Slave_Stick/01.Personal/02.Development/Legacy/`. **디스크를 꽂고** 경로를 인자로 줘야 한다 — 안 주면 바탕화면을 찾다가 안전하게 멈춘다.
  - ⚠️ 스크립트가 **클라우드플레어 API 실패에 stack trace 를 토하며 죽던 것**을 고쳤다(실제로 겪음 — 토큰 만료/일시 오류). 이제 `wrangler login` 안내와 함께 사람 말로 멈춘다.
