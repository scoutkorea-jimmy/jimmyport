...
**급식편의본부(`/krjam-fnc`)**
- **판단 대기 — 관리자 id/pw `admin`/`admin`**(2026-07-28): 사용자 지시로 그대로 뒀다. 대량 시도는 막았지만(IP당 10분 20회) **아는 사람은 누구나 편집할 수 있다**. 선택지: (a) 비밀번호만 추측 불가한 값으로 교체 (b) 홍보부 로그인으로 통합 (c) 편집 시 추가 확인 절차. **사용자 답을 받고 바꾼다** — 임의로 바꾸면 사용자가 못 들어간다.
