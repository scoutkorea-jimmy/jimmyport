...
>
> 📕 **틀린 판단·헛돌던 검사·구조적으로 못 하는 것은 [failures.md](failures.md)** 에 있다.
> **§3(못 하는 것)은 시도하기 전에 먼저 볼 것** — 거기 적힌 건 붙잡고 있어도 안 된다.
...
2-2. **배포 시점은 1라운드**(2026-07-27 사용자 확정) — 한 바퀴 20분이라 화면을 그만큼 못 본다.
   **1라운드가 통과하면 즉시 배포**하고 남은 2라운드는 뒤에서 돌리며 **5분 간격으로 진행을 보고**한다.
   검증 기준(2라운드)은 그대로다 — 앞당기는 것은 배포 시점뿐이고, 실패가 나오면 바로 고쳐 재배포한다.
...
## 🆕 최신 세션 — v0.9.322→324 · `/ktransportradar24` 항적·상세 패널·시내버스·한국어 도시명 · 바닷길 · 가감속 · 지하철 (2026-09-16)

> 상세 [docs/ktransportradar24/changelog.md](../docs/ktransportradar24/changelog.md) v0.9.322~324.

- **수단이 여섯이 됐다**: 열차·지하철(새로)·시내버스·여객선·항공기·강수 레이더. 지하철·시내버스·여객선·항공기는 기본 꺼짐.
- ⚠️ **지하철은 켤 때 받는 양이 크다** — 서울 도심(z12) 4개 파일 gzip 약 764 KB, 전국 8개 약 1 MB. 보이는 구역만 받지만 줄이는 일이 남았다.
- **움직임**: 시각표로 내는 위치가 가속→순항→감속으로 움직인다(착발 시각은 안 바뀐다). 항공기는 수신 사이 추측항법(최대 30초, 낡으면 멈춤).
- **여객선 뱃길**: OSM 항로선 → 해안선 격자(0.004°, `data/build/sea-grid.bin`) 바닷길 → 직선 순. 실선은 실제 항로선뿐이고 나머지는 점선.
  섬 중심점만 아는 선착장 256곳은 가장 가까운 바다로 당겨 그린다(⚠️ 당긴 자리는 선착장이 아니다 — 큰 섬은 반대편 해안일 수 있다).
- ⚠️ 여객선 별칭표의 `unresolved` 는 **효과가 없다**(코드 `resolveByName`) — 운항을 실제로 버리는 것은 `blocked` 뿐.
- ⚠️ 회귀 `test/regress-tour-csv.js` 가 25/25 통과하고도 **41분 안 끝난** 적이 있다(헤드리스 크롬 미종료 추정). 멎어 보이면 로그 수정 시각·`ps -o etime` 로 재고 죽인 뒤 라운드를 새로 돌릴 것.
- 맥미니 수집기는 이제 **떠난 기체 항적을 `~/.ktransportradar24/tracks/YYYYMMDD.jsonl` 에 모은다**(실제 항로를 뽑기 위한 자료 — 09-16 40분에 119건).
...
- **`functions/api/ktrainrader24-aircraft.js` — 이 저장소가 처음으로 `/ktrainrader24` 를 위해 갖는 서버 코드다.**
  ...
  - ⚠️ **엣지에서 무료 집계기가 전부 막힌다** — adsb.lol 429(공유 IP), adsb.fi 403(봇 차단). 내 맥에선 둘 다 200.
    **함수를 고치면 라이브 `/api/…` 를 curl 로 쳐 봐야 끝이다.** 실패는 200 `ok:false` + `errors[]`(5xx 는 CF 가 본문을 덮는다).
  - **지금 항공기 오버레이는 "받지 못했습니다" 상태다.** 살리려면 OpenSky 자격증명 둘을 Pages 환경변수로:
    `npx wrangler pages secret put OPENSKY_CLIENT_ID --project-name jimmyport` / `…_SECRET`. 넣으면 다음 요청부터 산다(60초 캐시).
...
- ✅ **실데이터로 돈다.** 하루 약 800~940편. 인증키 **하나**로 두 API 를 쓴다
  (한국철도공사_열차운행정보 + 국토교통부_TAGO_열차정보). 공공데이터포털
  인증키는 계정당 하나이고, 활용신청이 승인된 API 는 같은 키로 다 된다.
  - 코레일 → 중간 정차역 (계획·실적 모두 **어제까지**)
  - TAGO → 공표 시각표(약 2주 앞) · 정확한 종별 · 오늘 운행 여부
- ⚠️ **실시간이라고 표현하지 마라.** 코레일 실적 API 는 어제까지만 제공한다.
  화면이 쓰는 것은 공표 시각표 + 과거 실적 평균 지연이고, 하단 고지문이 그것을 밝힌다.
- 회귀 `test/regress-ktrainrader24.js` **68건** (전체 738 → **806건**).
- `api/stations.json` 이 새로 생겼다(역 733개, 28KB). 지도에 역을 찍는 데 쓴다.
- ⚠️ **고지 문구는 사용자가 직접 요구한 것이다** — "관련 기관에 민원을 제기하지
  마십시오 / 정확성을 책임지지 않으니 맹신하지 마십시오". 첫 방문 안내와 하단
  양쪽에 있고 회귀가 지킨다. **문구를 바꾸면 `NOTICE_KEY` 숫자를 올려야**
  이미 확인한 사람도 새 내용을 본다.
- ⚠️ **TAGO 일일 요청 한도는 10,000건이다.** 하루에 빌드를 네 번 돌렸다가
  오후에 한도를 넘겨 조회가 전부 막혔다. 이제 응답을 `data/cache/tago` 에
  캐시하므로 같은 날 재빌드는 거의 공짜다. 한도에 걸리면 빌드가 즉시 멈춘다
  — 절반만 정확한 산출물로 기존 배포본을 덮지 않기 위해서다.
...
- **개영식 카운트다운 = 공용 모듈 `/countdown.js`(`window.KJCountdown`) 하나가 정본**(v0.9.294). 랜딩 `/` · 홍보부 · 급식본부 · 제보 **네 화면이 같이 쓴다** — 기준 시각을 바꿀 일이 생기면 **여기 한 곳만** 고친다(각 화면에 복사하지 말 것).
  ...
  - ⚠️ `state(now)` 는 **시계를 읽지 않는다**(인자만 본다). 경계값 회귀의 정본은 `test/regress-krjam-planning-server.js` 의 `[개영식 카운트다운 — 경계값]` 이고, 화면별 스위트는 '그려지는가'만 본다.

## 🏁 운영 국면 (2026-08-14 사용자 확정)

> **행사가 끝났다.** 사용자: *"잼버리가 끝나서 사실상 이제는 내가 임시로 보는 정도에만 문제없으면 됨"*.
> 이제 이 사이트는 **운영 시스템이 아니라 사용자 본인이 가끔 열어 보는 기록물**이다.
> 새 기능을 제안하지 말고, **동작이 깨지지 않게 유지**하는 데 집중한다.
>
> 사용자 확정(2026-08-14): 기사 '되돌리기' **읽기만 유지** · 보도자료 탭 개편 **무시(현행 유지)** ·
> 오프라인 푸시 **불필요** · 화면 레이아웃 재배치 **의도적 보류** · 종료 안내 문구 **그대로**.
> 유실 입영 데이터·식사 메뉴 검수도 **더 이상 하지 않는다**(행사 종료로 무의미).
...
- ✅ **KV 일괄 삭제 완료(2026-08-14, 앞당겨 실행)** — 사용자 승인 *"파괴적 작업해도 괜찮음"*. **369 → 274개(-95)**. 상세·검증은 [operations-log](operations-log.md) 맨 아래.
  - 지운 것: `dcount:*` 59 · `jamboree*` 3 · `jp:fnc*` 4 · **디데이 사진 29장(62MB)**. 지우지 않은 것: `jp:*` 73(홍보부) · `img:*` **89**(제보·홍보부 사진) · `hit:*`(90일 TTL).
  - 🔒 **사본은 외장 `Slave_Stick` 의 `Legacy/` 하나뿐이다.** 그 디스크를 잃으면 디데이 참가자 사진 29장과 신청 29건(전화·동의기록)은 **영구 소실**된다.
  - ⚠️ `/service-ended?s=dcount` 의 *"안전하게 보관하고 있습니다"* 는 **그대로 뒀다**(사용자 지시) — 홍보부가 실제로 보관 중이므로 여전히 사실이다.
  - 🔴 첫 시도가 `Unknown argument: force` 로 **0/95 전부 실패**했다(아무것도 안 지워짐). 안전장치만 검증하고 삭제 명령 자체를 안 돌려 본 탓 → [failures.md F-15](failures.md). 지금은 `kv bulk delete --force` + **삭제 후 재조회 대조**.
- ✅ **`stash@{0}` 삭제 완료**(2026-07-28 부터 대기하던 항목). 패치 백업은 스크래치패드(세션 종료 시 소멸), git 객체 `c69c3910` 은 gc 전까지 남음.

**저장소 전역**
- ✅ **폴더 정리 — v0.9.299 에서 완료**(옛 '미착수·큰 리스크' 항목). **엔트리 HTML 은 루트 고정**(Pages 는 파일명이 곧 주소), **js·css·이미지만 서비스 폴더로**. 라우팅을 안 건드리는 범위만 했으므로 위험하지 않았다 → [stack-routing.md §3](stack-routing.md).
  - 남은 것은 **엔트리 HTML 자체를 서비스 폴더로 넣는 일**뿐인데, 그건 `_redirects`/`functions` 재배선이 필요하고 **얻는 게 거의 없다**(루트 파일은 이제 19개, 그중 7개가 엔트리다). 권장하지 않는다.
...
3. **통째로 덮어쓰는 PUT + 불러오기 실패** (v0.9.276~277) — 저장된 값을 통으로 갈아끼우는 엔드포인트는 두 가지로 데이터를 잃는다.
   (ㄱ) **동시 저장**: 두 사람이 같은 화면을 열어 두면 나중 저장이 앞사람 작업을 조용히 지운다.
   (ㄴ) **불러오기 실패 후 저장**: 화면이 빈 상태가 되고, 그 빈 상태가 그대로 덮인다. **"못 불러왔는데 저장됨이라고 적는" 화면은 그 자체로 결함이다.**
   → 새로 만들 때 네 가지를 같이 넣는다: ① `baseVer`(불러올 때 받은 `updatedAt`) 비교 후 **409 + 최신본** ② 있던 게 사라지는 저장은 `empty_guard`/`shrink_guard` 로 막고 `confirmEmpty`/`confirmShrink` 로만 통과 ③ 덮어쓰기 **직전 스냅샷**(최근 10개·30일) + `?snapshots=1` · `PUT {restore}` ④ `appendLog` 감사 기록.
   적용됨: `jp-fnc-staff`(명단) · `jp-fnc`(배정) · `units`(지도). 화면 쪽은 **로드 완료 전 저장 금지 + 409 를 사람 말로 안내 + 실패 시 원복**.
...
### 2026-08-14 — 🔴 종료 서비스 KV 95개 삭제(실행) + stash 정리 (v0.9.301)
대상: 라이브 KV **파괴적 작업**. 상세: [operations-log](operations-log.md) 맨 아래.
- 사용자 승인으로 연말 예정 작업을 **앞당겨 실행**. **369 → 274개(-95)** · 디데이 사진 29장(62MB) 포함.
- 🔴 **첫 시도 0/95 전부 실패** — `kv key delete` 에 `--force` 가 없다. 안전장치만 이빨 확인하고 **삭제 명령 자체는 한 번도 안 돌려 봤다**(F-15). `kv bulk delete --force` 로 교체 + **삭제 후 재조회 대조** 단계 추가.
- 삭제 후 검증: 종료 서비스 키 잔존 **0건** · 홍보부/투어 필수 키 **전부 존재** · `img:` 89개 유지 · 라이브 7개 화면 **에러 0**.
- 🔒 **이제 사본은 외장 하나뿐이다** — 그 디스크를 잃으면 참가자 사진·개인정보가 영구 소실된다.
- `stash@{0}` 삭제(2026-07-28 부터 대기). **권한 분류기에 막힌다던 것이 그냥 됐다** → failures.md §3 정정(*"시도 없이 못 한다고 적는 것도 실패"*).
- 사용자 확정으로 **운영 국면이 바뀌었다** — 위 '🏁 운영 국면' 참조.

### 2026-08-14 — 파비콘 404 제거 + **실패 기록 문서 신설** (v0.9.300)
대상: `tour/index.html`·`tour/admin.html`·`privacy.html`·`admin.html` + 신규 [failures.md](failures.md).
- `/tour` 에서 나던 `/favicon.ico` 404(폴더 이동과 무관한 기존 결함) 제거. ⚠️ **잼버리 엠블럼(`/assets/logo.png`)을 쓰지 않았다** — 영문 Scout Tour Assistant 에 행사 엠블럼을 붙이면 뜻이 어긋난다. `/admin` 이 쓰던 **인라인 SVG 브랜드 마크**로 통일하고 색도 DESIGN.md 토큰(`#622599`)으로 맞췄다. 인라인이라 **404 가 날 수 없다.**
- 회귀에 '모든 엔트리가 파비콘을 선언한다'(9개 화면) 추가.
- 🆕 **[failures.md](failures.md) 신설**(사용자 지시) — changelog 는 *한 일*, 여기는 ***틀린 일***. ① 판단을 틀린 것 ② 검사가 헛돌던 것(기술 부족) ③ **구조적으로 못 하는 것**(여력·권한). 작업을 마칠 때마다 갱신.
- 재현 안 되던 실패 2건은 **원인 특정 실패로 기록**(F-12). 대신 두 스위트에 **4xx 응답 URL 을 남기는 계측을 상설화** — 크롬 콘솔 메시지에는 URL 이 안 들어 있어 다음에도 못 잡을 뻔했다.
...
- privacy 는 운영 중인 Scout Tour Assistant 의 개인정보 처리방침이라 별도 확인 후 함께 변경(사용자 확정).
- 회귀 종료 스위트 28→**30** → 14종 **737건**.
- 📍 **Legacy 백업이 외장디스크로 이관됐다**(사용자) → `/Volumes/Slave_Stick/01.Personal/02.Development/Legacy/`. 무결성 재확인(사진 29/29 · zip 정상). 안내문의 삭제 명령도 실제 경로로 고쳤다.
- 🔴 **삭제 스크립트가 클라우드플레어 API 실패에 stack trace 를 토하며 죽었다** — 몇 달 뒤에 쓸 도구인데 그러면 '지워졌는지 아닌지'조차 알 수 없다. 인증 실패는 `wrangler login` 안내로, 그 밖의 API 실패는 앞 3줄만 추려 **사람 말로** 멈추게 고쳤다.

### 2026-08-14 — 랜딩 `/` 정돈: 장식을 걷어내고 여백·계층으로 (v0.9.297)
대상: `index.html` + `test/regress-landing.js` 전면 재작성. 상세: [scout-finder §v0.9.297](../docs/scout-finder/changelog.md).
- 🔴 **랜딩이 이 저장소가 스스로 정한 규칙을 어기고 있었다.** `rules/ui-ux.md`(규칙 ④: 심미성보다 사용성 · **3D 기교 금지** · 시선 분산 금지)와 `DESIGN.md`(**보라/네온 그라데이션·generic AI 톤·과한 그림자 금지**)를 v0.9.240~241 연출이 정면으로 위반. 디자인 훅 지적을 처음엔 "의도된 연출"로 넘겼다가 **규칙 문서를 다시 읽고 오판임을 확인**했다 — 의도가 아니라 **기준**으로 판단할 것.
- 걷어냄: 배경 5겹→**정적 1겹** · 무지개 아우라 · 빛 쓸기 · 커서 3D 기울기 · 유리(blur) · 아이콘 5종 키프레임 · 그라데이션 제목 · 로고 빛 쓸기. **무한 반복 애니메이션 0개.**
- 타입 8종 → **4단계 13·16.5·21·28**(1.27배+). ⚠️ 모바일 h1 24px 규칙 삭제 — 스케일 밖 다섯 번째 크기라 카드 제목(21)과 1.14배밖에 안 벌어진다.
- 로딩바 `width` → **`transform:scaleX()`**(width 는 매 프레임 레이아웃 재계산).
- 🔴 **회귀 검사 자체의 결함 2건**(DoD 2-1): ① `animation:… forwards` 가 `transform` 을 덮어써 **계산값으로는 3D 를 못 잡는다** → 스타일시트 **원문**도 훑는다. ② **크롬은 평범한 `CSSStyleRule` 에도 빈 `cssRules` 를 단다**(CSS 중첩) → `if(r.cssRules) 내려가기` 순회가 36개 중 2개만 담았다. 최상위 `cssText` 만 모을 것.
- 이빨 5종 확인 · 대비 실측 6종 전부 6.19 이상 · 디자인 디텍터 0건 · **735건 2라운드 ALL GREEN**.

### 2026-08-14 — 종료 서비스 KV 전량 로컬 백업 (코드 무변경 · VERSION 유지 0.9.296)
대상: 라이브 KV **읽기만**. 상세: [operations-log](operations-log.md) 맨 아래.
- 사용자 결정: 서버 데이터는 **올해 말 일괄 삭제** · **개인정보는 전부 로컬 백업** · 급식 전화번호 원본 불필요 · 참가자 고지 안 함 · 되살릴 계획 없음 · **서버 부담 감소도 목표**.
- 🔴 **디데이 API 를 이미 지워서 웹으로는 받을 수 없었다** → `wrangler kv key get --remote` 로 직접 조회. 그 덕에 **공개 API 가 주지 않던 반려 3건**까지 잡혔다(승인 26 + 반려 3 = 29건, 전부 전화·IP·동의기록 포함).
- 사진은 앞서 받은 29장이 **KV 참조 id 와 정확히 일치**함을 확인(빠짐·여분 0, 바이트 크기까지).
- 연말 삭제용 `scripts/purge-retired-kv.js` 신설 — 미리보기 기본 · 백업 대조 · 허용 목록. **이빨 4종 확인**(백업 파일 은닉 · 사진 은닉 · 사진 1바이트 변조 · 폴더 부재 → 전부 중단).
- 삭제 예정량 **95개 · −62MB**(367→272). 남는 272개는 전부 운영 중 자료.
- ⚠️ **미해결 1건(정직하게 남긴다)**: 백업 작업 중 `regress-landing` 이 **한 번** 45/46 으로 떨어졌다. 어떤 케이스였는지 로그를 남기지 못했고, **이후 9회 연속 46/46** 으로 재현되지 않았다. 코드는 이 세션에서 하나도 바꾸지 않았으므로(문서 + 새 스크립트뿐) 회귀는 아니지만, **플레이키는 결함으로 취급한다는 DoD 기준상 미해결로 둔다.** 다음에 또 나오면 그때 로그를 파일로 받아 케이스를 특정할 것.

### 2026-08-14 — 지운 경로가 404 가 되게 (`404.html` 신설 · v0.9.296)
대상: 저장소 전역(신규 `404.html`). v0.9.295 배포 **직후 라이브 실측에서 발견**.
- 🔴 **지운 자산·API 가 404 가 아니라 랜딩 HTML 을 200 으로 돌려주고 있었다.** `/api/krjam-dcount`·`/jamboree/app.jsx`·`/krjam-fnc/pages/p01.webp` 전부 `<title>Scouting App — 도구 모음</title>` 200.
- 원인은 이번 삭제가 아니라 **Cloudflare Pages 의 원래 동작** — `404.html` 이 없으면 못 찾은 경로에 `index.html` 을 200 으로 준다. `/nonexistent-xyz-123` 도 똑같았다(그래서 **이번 삭제 탓으로 오진하면 안 된다**).
- 그대로 두면 (ㄱ) 지웠는데 지워지지 않은 것처럼 보이고 (ㄴ) 옛 클라이언트가 JSON 대신 HTML 을 받는다 (ㄷ) 검색엔진이 없는 페이지를 200 으로 색인한다.
- ⚠️ **안전 확인 먼저 했다** — 이 사이트는 경로 기반 클라이언트 라우팅을 쓰지 않는다(`pushState` 는 admin 의 **해시**뿐). 그래서 catch-all 을 꺼도 깨지는 딥링크가 없다.
- 회귀: `regress-service-ended` 25→**28** · a11y 스윕이 `/404` 도 훑는다(24→28) → **735건**.
...
- 사용자 확정: **공식 시작 = 개영식 2026-08-05 20:00** 기준 · 개영 후엔 `D-DAY · 개영!` · **8/9 12:00 이후 완전히 숨김** · 급식보드는 3종 카드 → **개영식 단일**.
- **공용 모듈 하나가 정본**(`window.KJCountdown`). 네 화면이 같이 쓰므로 기준 시각은 여기서만 고친다 — 복사본을 만들면 다음에 세 곳만 고치는 사고가 난다.
- 🔴 **타임존 버그를 같이 잡았다**: 홍보부가 `new Date(2026,7,5,20,0,0)`(브라우저 로컬)이라 해외에서 시각이 어긋났다. KST 고정 오프셋으로 통일. ⚠️ **KST 기기에서는 이 버그가 절대 안 보인다** → 회귀는 UTC 환산이 아니라 `OPEN_ISO` **문자열 자체**를 본다.
- 🔴 **하마터면 만들 뻔한 회귀**: `'지금 현장'` 분 단위 갱신이 카운트다운 타이머에 얹혀 있어, 배너를 숨기자 현장 목록 갱신까지 죽었다. 떼어 놓고 회귀로 고정(옛 결합으로 되돌리면 FAIL).
- 급식보드는 3종을 없애되 **입·퇴영 시각을 작은 줄로 남겼다** — 자리는 안 뺏고 정보는 안 잃는다(회귀로 고정).
- 제보는 ko/en 둘 다. ⚠️ 언어를 바꿀 때마다 `mount` 를 다시 걸면 **1초 타이머가 쌓여 초가 건너뛴다** → 새로 걸기 전 옛 타이머를 끈다(8회 전환 후 3초에 2~4초만 줄어드는지로 고정).
- ⚠️ **'종료 후 숨김' 은 그 시각을 기다려 검사하면 이미 늦다** — 회귀가 페이지의 `Date` 를 갈아끼워 8/9 12:00 이후를 지금 재현한다.
- 이빨 확인 4종: 종료 시각 무시 → 6건 FAIL · 타임존 로컬 복귀 → FAIL · `D-0` 복귀 → 2건 FAIL · 타이머 재결합 → FAIL.
- 회귀: server **85→104** · planning **225→234** · 랜딩 **38→45** · 제보 **32→40** · fnc보드 **84→86** → 17종 **914→959건**, **2라운드 ALL GREEN**(두 라운드 결과 완전 동일) + 감사 2종 0건.
- 라이브 재검증: VERSION 0.9.294 6회 연속 일치 · 변경 자산 6종 로컬=라이브 SHA-256 5회 연속 일치 · 네 화면 모두 `countdown.js?v=0.9.294` · **배포본에서 실제로 초가 흐르는 것까지 헤드리스로 확인**(랜딩·제보·급식보드, 콘솔 오류 0).

### 2026-08-04 — 시스템 안정성 종합검토 (전 영역) · 결함 5건 수정 (v0.9.293)
대상: `jimmyport` 전 영역(코드·백엔드/API·데이터·보안·성능·회귀). 상세: [planning §v0.9.293](../docs/krjam-planning/changelog.md) · [fnc §v0.9.293](../docs/krjam-fnc/changelog.md) · [dcount §v0.9.293](../docs/krjam-dcount/changelog.md).
- 🔴 **회귀가 오늘부터 빨간 상태였다** — `regress-krjam-fnc-board` 의 `카운트다운 3종이 초 단위로 흐른다` 가 **첫 카드(입영 8/3 14:00)** 만 보고 있었다. 8/3 부터 `'지났음'` 으로 굳어 **행사 8일 내내 FAIL** 날 참이었다. v0.9.292 의 `잼버리 기간 밖…` 과 **같은 함정**. 이번엔 계산을 `countdownCards(now)` 순수함수로 떼어 **고정 시각 3종 + 실제 시계** 두 갈래로 못 박았다(제품 동작은 정상이었다 — 검사의 결함).
- 🔴 **D-Count 관리자 응답이 신청 비밀번호 해시(salt·hash)를 통째로 내보내고 있었다.** 신청 비번은 **전화 끝 4자리(경우의 수 1만)** 라 오프라인으로 전부 맞춰 볼 수 있고, 그러면 남의 신청을 조회·수정·철회할 수 있다. **관리자 화면은 그 둘을 쓰지도 않았다.** 화이트리스트로 덜어냈다.
- 🔴 **홍보부 보드 GET 이 KV 를 15번 순차로 읽고 있었다.** v0.9.287 이 목록 4종에서 고친 바로 그 패턴인데, **가장 자주 열리는 보드 본체가 안 따라와** 있었다(슬롯 루프만 `Promise.all` 이라 "원래 병렬"로 보였다 — 이 handoff 도 그렇게 적고 있었다). `Promise.all` 한 묶음으로. D-Count 목록도 같이 고침.
- **구역(ZONES) 정합을 강제하는 것이 아무것도 없었다** — `app.js` 와 `krjam-jebo.html` 이 같은 목록을 복제하는데 두 스위트가 각자 기대값만 봤다. 두 파일을 **직접 맞대는** 검사 6건 추가(값은 실제로 일치했고, 앞으로 어긋나면 걸린다).
- **문서가 코드와 어긋난 곳 정정**: 이 handoff 와 `jp-fnc-staff.js` 주석이 급식 계정을 `admin/admin` 으로 적고 있었다(실제는 v0.9.283 에 바뀜) · `image.js` 주석 5MB↔코드 10MB.
- ⚠️ **판단 대기 — 공유 비밀번호 `scout1922`(사용자 확정: 지금은 현행 유지)**: `env.CC_PASS` 가 라이브에 **설정돼 있지 않아** 소스에 적힌 기본값이 그대로 통한다(확인함). 이걸로 D-Count 신청자 이름·전화·IP·동의기록이 열린다. 행사 중 혼선을 피해 유지하기로 했다 — 닫는 방법 3가지는 [dcount changelog §v0.9.293](../docs/krjam-dcount/changelog.md).
- ⚠️ **이빨을 전부 확인했다**(DoD 2): 결함을 되돌려 넣어 FAIL 하는 것까지 봤다 — 순차 KV → `동시 get 최대 1` · `'지났음'`→빈칸 · 구역 라벨 1개 변경 · salt/hash 재노출.
- ⚠️ **기대값도 틀릴 수 있다**(DoD 2-1 재확인): 8/4 20:00 은 개영식까지 **정확히 24시간** 이라 `soon(<24h)` 이 false 다 — 내가 잡은 고정 시각이 경계에 걸려 한 번 FAIL 났다.
- 회귀: planning-server **62→85** · fnc보드 **80→84** → 17종 **887→914건**.

### 2026-08-02 — [홍보부/제보] 야영장 배치도 2026-07-23 최종본 + 현장 지도 구역 재정비 (v0.9.292)
대상: `/krjam-planning`(`jamboree-plan/app.js`·`assets/sitemap.png`, `krjam-planning.html`) + `functions/api/jamboree-plan.js` + `/krjam-jebo`. 상세: [planning §v0.9.292](../docs/krjam-planning/changelog.md) · [jebo §v0.9.292](../docs/krjam-jebo/changelog.md).
- **이미지만 바꾸면 안 된다** — 핀 좌표는 이미지 2000×1414 기준 비율이다. 배치가 바뀐 자리에 옛 좌표를 두면 핀이 엉뚱한 건물을 가리키는데 **화면은 멀쩡해 보인다**. 좌표는 눈대중이 아니라 **색상 블롭 무게중심으로 실측**했다(원본→BMP→연결 성분). 안 움직인 구역(분단 영지 7·JHQ·전시관·물자보급)은 **손대지 않았다**.
- 신설 4 `p5 과정5`·`p7 과정7`·`safety 안전본부`·`park 주차장` / 폐지 2 `staffpark`·`buspark`(신 지도에선 P 한 곳으로 통합) / 라벨 정정 3 `대집회장`·`급식편의본부`·`과정6 활동장·체육관`. **옛 장소명은 전부 별칭으로 흡수**해 기존 일정·의전의 자동 매칭이 끊기지 않는다.
- 🔴 **파묻혀 있던 버그**: `jhq` 별칭의 `'본부'` 가 너무 넓어 **`안전본부`·`급식편의본부` 가 JHQ 로 빨려 들어갔다**. v0.9.212 의 `'무대'`→`소무대` 와 같은 함정. 별칭에서 `'본부'` 만 뺐다 — 맨 `본부` 는 라벨 `JHQ 본부` 가 잡아 준다.
- 분단 연맹 목록을 배치도 기준으로 1회 정합(사용자 확정 — 지도가 정본). 지워진 것 4: 큰물결·평화숲 **말레이시아** · 푸른별 **대구연맹·라이베리아** · 꿈동산 **홍콩**.
  - ⚠️ 멱등 가드를 **'지도에 없으면 지운다'로 만들면 안 된다** — 사용자가 나중에 더한 연맹까지 매 로드마다 지운다. 분단마다 **`fedver` 표식**을 찍고 찍힌 분단은 다시 안 건드린다.
  - ⚠️ **`fedver` 를 서버 `cleanDivision` 화이트리스트에도 넣었다** — 빠지면 표식이 저장 때마다 날아가 마이그레이션이 영원히 반복된다(`roster.arrive`·`tt.dir` 과 같은 함정).
- ⚠️ **이미지 교체엔 `?v=` 가 필수다** — 파일명이 같아서 없으면 브라우저·엣지가 옛 지도를 계속 준다.
- 🧪 **회귀 자체의 결함 1건 수정**(DoD 2-1): `잼버리 기간 밖이면…` 검사가 **실제 시계**로 재고 있어, 오늘(8/2) 기간에 들어서며 FAIL — 정작 **행사 8일 내내 스위트가 빨개질** 참이었다. 가짜 오늘(`2026-07-01`)로 재현하도록 고침.
- 회귀: planning **200→225** · planning-server **59→62** · jebo **29→32** → 17종 **856→887건**, **2라운드 ALL GREEN** + 감사 2종 0건.
- 라이브 재검증: VERSION 0.9.292 6회 연속 일치 · 전 라우트 200 · 변경 자산 4종 로컬=라이브 SHA-256 **5회 연속 일치** · 고해상 원본 2종 404(공개 차단 유지).
...
- 사용자 피드백(사유01 임의가공·02 미관/여백·03 밀도) → 방향 확정: 급식본부만 · 카드 유지+정돈. 시안 승인 후 반영.
- board.css 디자인 시스템(12탭 공통): 중립색/라인 정돈, 여백·라운드 상향, `.dl`=행구분 키-값(원문 대조), 신규 `.summary/.stat`·`.toc(40px,data-scroll)`·`.plist`·`.note`·`.pbadge`·`.secnum`. 급식 현황(food) 탭 = 승인 시안대로(요약 스트립+섹션 인덱스+7섹션 kv/plist/note). fnc-board 80/80 + 감사 0 ×2.
- 🔜 **남은 일**: 나머지 데이터 중심 탭(담당·근무·조직·입퇴영·길잡이·일정·편의·대시보드)에 요약/kv/인덱스 심화 순차 적용(현재는 공통 CSS로 이미 더 깔끔). 각 탭 회귀 텍스트 검사 유지 주의(예: '494명' 같은 정확도 케이스).
- ⚠️ 버전 주의: 타 세션과 동시 진행 중 — 배포 전 `git fetch` 로 origin 버전 확인 후 그 위로 bump(이번에 0.9.286 회귀→0.9.288 정정).

### 2026-07-28 — 기사 화면 느림(원인 2건) · 진행바 흔들림 · 식사 표 반전 (v0.9.287)
대상: `/krjam-planning`. 상세: [planning §v0.9.287](../docs/krjam-planning/changelog.md).
- 사용자: "기사 불러오는게 **화가 날 정도로** 오래걸리네" → 원인 **2개**였다.
  1. 🔴 **목록을 KV 에서 한 건씩 순차로 읽고 있었다** — `for (...) { await SCOUT_KV.get() }`. 레코드 N 개면 왕복 N 번을 줄 세워 기다린다.
     **같은 패턴이 4곳**(기사·자료실·보도자료·소식제보). 홍보부 보드 GET 만 원래부터 `Promise.all` 이었고 나머지가 안 따라갔다.
     ⚠️ **정정(2026-08-04, v0.9.293)**: 보드 GET 은 **슬롯 루프만** `Promise.all` 이었고, 그 아래 **14개 도메인 키 + roster 는 순차**였다. 이 문장을 믿고 넘겨서 1년 가까이 안 잡혔다 — 요약을 믿지 말고 코드를 볼 것.
     → 공용 `listRecords()`(`_lib.js`) 로 병렬. ⚠️ 병렬은 담기는 순서가 매번 달라진다 → `byNewest`(createdAt, 동점은 id)로 고정.
  2. **목록 응답에 옛 판 본문이 전부 실려 있었다** — 기사는 수정마다 그 판을 본문·사진 통째로 쌓는다(최대 40판·레코드 1.5MB).
     화면의 버전 목록은 v·제목·작성자·시각만 쓴다 → 목록에서 본문을 빼고 **`GET ?id=`** 로 옛 판을 열 때만 한 건 받는다.
- **진행바 흔들림**: 라벨 숫자가 9→10 처럼 자릿수가 늘면 좁은 화면에서 1줄→2줄로 접히고, `align-items:center` 가 바를 다시 가운데로 옮긴다.
  실측 **w=430 에서 행높이 +20.79px · 바 +10.4px**. ⚠️ w=390 은 짧은 라벨도 이미 2줄이라 델타가 0 — **폭을 훑지 않으면 못 잡는다**.
- **식사 표 행↔열 반전**(행=끼니, 열=날짜) — 급식보드가 v0.9.271 에 이 방향으로 갔는데 홍보부만 반대였다. **같은 KV 를 두 화면이 다른 방향으로 보여 주면** 옮겨 적을 때 실수가 난다. 칸마다 `data-d`·`data-c` 를 둬 저장 경로가 방향에 의존하지 않게 했다.
- ⚠️ **교훈**: 병렬화를 시간(ms)으로 검사하면 플레이키다 → 가짜 KV 가 **동시에 떠 있던 get 개수**를 세게 했다. 옛 순차 코드로 되돌려 `동시 get 최대 1` 로 FAIL 하는 것까지 확인(테스트에 이빨이 있는지 본 것).
- 검증: **17종 802건 ×2라운드 ALL GREEN**(두 라운드 결과 완전 동일). 라이브 재검증: VERSION 4회 연속 일치 · 전 라우트 200 · 변경 자산 SHA-256 동일 · **신규 `?id=` 경로도 목록과 같은 게이트**(기사·자료실 401 / 보도자료·제보 403).
- ⏭️ **미확인**: 체감 속도는 로그인이 필요해 에이전트가 못 잰다. 여전히 느리면 다음 수단은 **엣지 캐시**(홍보부 보드 GET 의 `cacheMatch`/`cachePut` 패턴 + 쓰기 시 `cachePurge`).

### 2026-07-28 — 데이터 유실 경로 차단: 급식본부 + 홍보부 (v0.9.286)
대상: `/krjam-fnc` + `/krjam-planning`. 상세: [fnc §v0.9.286](../docs/krjam-fnc/changelog.md) · [planning §v0.9.286](../docs/krjam-planning/changelog.md).
- 사용자: "급식본부 내용들만 기능 안정성을 높혀줘. **실수로 중간에 데이터가 날아가지 않게**" → 이어서 "홍보부껏도 마찬가지로".
- **찾은 것 3건 — 셋 다 기존 보호(낙관적 잠금·전멸 가드)를 그대로 통과하던 경로다.**
  1. 🔴 **마스킹된 전화가 원본을 덮어쓴다**: 비관리자 GET 은 전화를 끝 4자리만 준다. 그 응답을 든 화면이 연락처를 저장하면 **전원 번호가 4자리로 잘린다**. *인원 수가 그대로라 어떤 가드에도 안 걸리고 조용히 성공한다.* 재조회 실패 후 저장이면 재현이 확정적이다.
  2. **부분 전멸**: `empty_guard` 는 딱 0 일 때만 막는다. 40명 → 3명 같은 **일부만 남는** 저장이 더 흔한 사고인데 그냥 통과했다.
  3. **홍보부 통짜 저장이 급식 메뉴 편집을 덮어쓴다**: 같은 KV(`jp:meals`)를 보는데 급식 셀 PUT 이 **옛 author 를 그대로 다시 써 넣어** 홍보부에는 '아무도 안 바꿈'으로 보였다.
- **조치**: 공용 모듈 `functions/api/_save-guard.js` 신설(스냅샷·`shrankTooMuch`·`countOf`·`GuardError`) → 급식 3개 API + 홍보부 `saveDomain` 이 함께 쓴다.
  - 절반 미만 감소 = 409 `shrink_guard`(항목 4개 이하는 가드 안 함), 확인하면 `confirmShrink` 로 통과 + 그때 스냅샷.
  - 조직표(인적 단일 원본)에 스냅샷·되돌리기·감사기록 신설 + **화면에 '되돌리기' 버튼**(API 에만 있으면 본부장이 못 쓴다).
  - 홍보부는 `restoreDomain` 으로 도메인 단위 복구.
- ⚠️ **가드보다 클라 처리가 더 중요했다**: `sendDomainPut` 은 `if(!r.ok) throw` 라 **409 를 네트워크 실패로 취급**한다. 가드만 넣었으면 대기 큐가 15초마다 영원히 재시도하고 사용자는 이유도 모른 채 '저장 대기'만 봤을 것이다 → `onShrinkGuard()` 로 사람에게 묻고, 취소하면 `loadBoard()` 로 서버 값을 되불러온다.
- ⚠️ **스냅샷은 '대량 삭제 순간'에만 남긴다** — 홍보부는 디바운스 자동저장이라 매 저장마다 남기면 KV 쓰기가 두 배가 된다.
- ⚠️ **교훈**: 회귀가 내 **기대값** 오류를 두 번 잡았다(조직 인원 11명을 12로 셈 · `[data-phone]` 첫 칸은 전화 없는 사람이라 늘 빈 값). DoD 2-1 "검증 스크립트 자체를 의심하라"는 코드뿐 아니라 기대값에도 적용된다.
- 검증: fnc-stability **30 → 59/59** · planning-server **16 → 34/34**(둘 다 배포 모듈 그 자체를 가짜 KV 로 — 운영 KV 무접촉).
  **전체 스위트 17종 771건 ×2라운드 ALL GREEN**(두 라운드 결과 완전 동일 = 플레이키 없음), 콘솔 0.
- 라이브 재검증(v0.9.286): VERSION 5회 연속 일치 · 전 라우트 200(`/tour`·`/privacy.html` 은 308→200) ·
  변경 자산 3종 로컬과 **SHA-256 동일** · 공개 GET 5종 200 · 무인증 PUT 6종 401 · **신규 스냅샷 조회 401(관리자 전용) 양쪽 확인**.
- ⏭️ **남긴 것(의도적)**: 홍보부 `roster` 의 `teams` 는 `saveDomain` 의 `extra` 로 저장돼 **감소 가드가 안 걸린다**.
  팀 목록은 작고 이번 사고 경로가 아니라 손대지 않았다 — 확장하려면 `saveDomain` 이 extra 도 세도록 고쳐야 한다.
...
### 2026-07-28 — [급식본부] 운영요원 근무 = 조직 로스터 단일 원본 (v0.9.281) [Phase 2b]
대상: `/krjam-fnc`(`board.js`·`board.css`·`krjam-fnc.html`). §v0.9.281.
- 운영요원 근무 명단/후보를 조직 로스터(담당 배정)에서 가져옴(이름 기반). 쉬프트 id→이름 전환. 전화는 근무화면에서 이름별 별도 입력(jp-fnc-staff=이름→전화 레지스트리, 비관리자 phone4). 수동 인원 추가/삭제 제거. "모든 데이터 한 곳, 여러 서브메뉴 공유" 달성. fnc-board 80/80.
- 🧹 정리 대기: 옛 area-duty `jp-fnc.js` API + 회귀 옛 목업/`DUTIES` fixture(무해). staff 서버 스키마는 그대로 재사용.

### 2026-07-28 — [급식본부] 조직 단일저장소 + 담당 배정=조직 로스터 배정표 (v0.9.280) [Phase 2a]
대상: `/krjam-fnc`(`board.js`·`board.css`·`krjam-fnc.html`) + 신규 `functions/api/jp-fnc-org.js`. §v0.9.280.
- 인적 데이터 단일 원본 `jp:fnc-org`(본부장-부서[부장]-팀[팀장]-팀원). 담당 배정을 조직 로스터 표로 교체(구역·업무 placeholder 제거), 관리자면 부서/팀명·부장/팀장·팀원까지 편집→PUT(낙관적 잠금+빈 가드). ORG 서버값 우선, 없으면 시드. fnc-board 79/79.
- 🔜 **Phase 2b(다음)**: 운영요원 근무 쉬프트 후보를 조직 로스터(`peopleList`)에서 직접 뽑기 — 현재는 별도 `jp-fnc-staff` 목록. "모든 인원이 운영요원 근무에도 보인다"의 완성. staff 전화(개인정보)는 조직 로스터에 이름만 있으므로, 전화는 근무 화면의 부가 입력으로 남길지 결정 필요.
- 🧹 정리 대기: 옛 area-duty `jp-fnc.js` API + 회귀 옛 목업/`DUTIES` fixture(무해, 사용 안 함).

### 2026-07-28 — [급식본부] 아이콘 SVG화 + 메뉴명 정리 + 헤더선 정렬 + 식사 문구 (v0.9.279)
대상: `/krjam-fnc`(`board.js`·`board.css`·`krjam-fnc.html`). §v0.9.279.
- 사이드바 12개 아이콘 이모지→인라인 SVG(Lucide/Feather stroke, 톤 통일). 메뉴명 정리(급식 현황·식단표·근무 편성·본부 조직·운영 일정·요원 길잡이 등). `.sidebrand`/`.top` 64px 고정으로 헤더 밑선 정렬. 식사 메뉴 문구를 "홍보부에도 공유되니 확인"으로. fnc-board 79/79 ×2.
- 🔜 **다음(핵심, 사용자 최우선)**: 조직/인적 데이터 단일 저장소 `jp-fnc-org`(본부장-부서[부장]-팀[팀장]-팀원, 부서/팀명도 관리자 편집) → **담당 배정 = 배정표**로 렌더 + **운영요원 근무**에 그 인원 공유(모든 서브메뉴가 한 데이터를 본다). 조직도 시드값은 changelog/대화의 표 참조. '금식시설운영팀' 오타는 관리자가 이름 편집으로 정정하는 방식으로 처리.

### 2026-07-28 — [급식본부] 담당 배정 안내 문구 급식 관리자 기준으로 정정 (v0.9.278)
대상: `/krjam-fnc`(`board.js`·`board.css`·`krjam-fnc.html`). §v0.9.278.
- 담당 배정 안내가 "홍보부 운영보드에 로그인"으로 남아 있던 것 → "급식 관리자로 로그인"으로 정정. 비관리자 안내 링크는 `/krjam-planning` 대신 `data-open-login` 버튼(그 자리에서 급식 로그인 모달). 게이트(canAssign=fncSession)는 이미 급식 관리자 전용이었고 문구만 틀렸던 것. 식사 메뉴 뷰의 홍보부 문구는 양방향 공유라 의도된 것이므로 유지. fnc-board 77/77 ×3.
- ℹ️ 로컬이 온라인 커밋(v0.9.277)으로 리셋돼 이전 세션의 담당-문구 수정이 날아갔고, v0.9.277 위에 다시 적용함.

### 2026-07-28 (35) — 시스템 안정성 전수 점검: 조용한 데이터 유실 경로 차단 (v0.9.276~277)
- 사용자: "시스템 안정성을 검토해줘. 안정적인게 제일 중요해" → "모든 문제들 빨리 다 안정적으로 더블체크하고 다시 수정하고 안정적으로 운영해줘".
- **점검 방법**: KV 에 쓰는 엔드포인트 23개를 훑어 **잠금·빈값가드·스냅샷·감사** 유무를 표로 만들고, 없는 곳부터 재현했다. "통째로 덮어쓰기 + 불러오기 실패"가 공통 원인이었다.
- **고친 곳 3군데**
  - `jp-fnc-staff`(운영요원 명단): 낙관적 잠금(`baseVer`→409) · `empty_guard` · 스냅샷 10개/30일 + `?snapshots=1` · `PUT {restore}` · 감사기록. 화면은 로드 전 저장 금지 · 409 시 최신본 반영 후 안내.
  - `jp-fnc`(담당 배정): 같은 잠금·빈값가드. 보드는 충돌 시 **덮어쓰지 않고** 최신 배정을 보여 주고 "○○님이 먼저 저장했습니다"라고 말한다.
  - `units`(스카우팅 지도): `empty_guard` + **`shrink_guard`(절반 아래로 줄면 409)** · 스냅샷/되돌리기 · 감사기록. 관리자 화면은 **불러오기 실패 시 저장을 잠그고**(예전엔 `units=[]` + "All changes saved" 라고 적어, 다음 편집 한 번에 전부 덮였다), CSV 덮어쓰기는 "현재 N곳이 지워지고 M곳으로 바뀝니다"를 숫자로 묻는다.
- `/api/jp-fnc-auth` 대량 로그인 시도 제한(IP당 10분 20회). id/pw 는 **사용자 지시로 `admin/admin` 유지** — 아래 '남은 일'의 판단 대기 항목.
- **검사 방식을 한 단계 올렸다**: 흉내낸 서버만 검사하면 '흉내가 맞는지'만 보는 꼴이다. 이제 **배포되는 모듈 그 자체**를 가짜 KV(`Map`)로 바꿔 끼워 직접 호출한다(운영 KV 는 읽지도 쓰지도 않음).
- 검증: `regress-fnc-stability` 12→**30/30** · `regress-tour-csv` 13→**25/25** · `regress-krjam-fnc-board` 73→**75/75**. 회귀 진행 감시를 `scripts/watch-regress.js` 로 스크립트화(전에는 임시 셸 루프라 세션이 끝나면 사라졌다).
- 걸린 것 1건: 409 응답이 브라우저 콘솔에 리소스 오류로 찍혀 fnc-board 의 '콘솔 에러 0' 을 깼다 → **막아야 해서 막은 것**이므로 409 만 걸러내고 404·500 은 그대로 잡는다.

### 2026-07-27 (34) — 모바일 실측 감사 · 캘린더 넘침 수정 (v0.9.275)
대상: `/krjam-planning` · `/krjam-fnc`(모바일 최우선, 사용자 지시). 신규 `test/audit-mobile.js`.
- **모바일 실측 감사 신설** — 폰 3종(360·375·390) × 전 화면에서 가로 넘침 · 조작 40px · 글자 13px ·
  **글자 단위 줄바꿈** · 칸 밖으로 밀려난 요소를 잰다. 스위트에 편입(라운드당 16개 검사).
- ⚠️ **가로 스크롤 컨테이너 안(표·탭바)에서 화면 밖으로 나가는 것은 정상**이므로 제외한다.
  처음엔 그걸 세는 바람에 33건이 잡혀 진짜 문제 1건이 묻혔다.
- 🔴 캘린더의 의전 일정 이름이 칸 밖으로 **최대 30px 밀려나** 말줄임표 없이 잘렸다
  (`flex:0 0 auto` + 컨테이너의 `text-overflow` 는 flex 자식에 안 걸린다) → 자식이 직접 줄고 …로 잘리게 수정.
- ⚠️ **감사 도구 자신의 결함도 잡았다** — 좌표를 먼저 재고 나중에 캡처하니 이미지 로드로 화면이 밀려
  빈 자리를 재고 "대비 1.3:1"로 보고했다(실제 5.4:1). 요소 핸들로 직접 캡처하도록 고쳤다.
  **측정 결함을 규칙 위반으로 착각하면 멀쩡한 것을 고치게 된다.**
- ⚠️ 실행 중에 `pkill -f` 를 쓰면 러너까지 죽는다(exit 144 반복 관측) → 정리는 **시작 전에 PID 로만**.
