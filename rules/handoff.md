...
1. **회귀 전체 스위트를 2회 연속 통과**한다(2026-07-28 사용자 확정: 3회→2회). 관련 없어 보여도 매번 전부 돌린다.
   - `planning 225 · nav 19 · server 62 · motion 33 · press 31 · news 112 · jebo 32 · fnc(플립북) 92 · fnc보드 80 · 랜딩 38 · a11y스윕 36 · admin 42 · tour-csv 25 · fnc안정성 59 · audit-mobile 1` = **887건** + 감사 2종(`audit-krjam-planning`·`audit-krjam-fnc`) (한 바퀴 약 6분)
...
**급식편의본부(`/krjam-fnc`)**
- **결정됨(2026-07-28) — 관리자 id/pw `admin`/`admin` 현행 유지.** 사용자 확정: *"급식관리자는 급식본부꺼라 급식본부 본부장님이 별도 요청 전까지는 그대로 유지"*. **급식본부 본부장(심호웅)의 요청이 있을 때만** 바꾼다 — 에이전트가 임의로 교체하지 않는다(교체하면 본부 쪽이 못 들어간다).
