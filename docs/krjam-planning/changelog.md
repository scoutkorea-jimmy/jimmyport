...
### 16.104 v0.9.254 — 기사 목록 정렬 · 거르기 + 목차에 협조부서·취재 담당자

**① 기본 정렬 = 우선순위 높은 순** (사용자 지시)
- 목록을 열면 **별점이 높은 기사가 맨 위**에 온다. 같은 우선순위면 최신 글이 위.
- 머리글(번호 · 제목 · 작성자 · 작성일 · 우선순위 · 단계)을 누르면 그 기준으로 정렬하고, 다시 누르면 반대로. 현재 기준에는 ▲▼ 표시.

**② 거르기** — 단계 · 영문 · 담당자 · 협조부서 고르개 + 제목/태그/담당자 검색.
- 거르는 중에는 통계바에 **보이는 건수**를 함께 띄우고, 전체 선택은 **걸러 놓은 것만** 고른다(안 보이는 기사가 딸려 들어가면 사고다).

**③ 목차에 협조부서 · 취재 담당자** — 협조부서 열 신설(2개까지 칩 + `+N`), 작성자 칸에 `취재 ○○○ / 사진 ○○○`.

**보기 정리** — 열이 늘어 표가 화면을 넘던 것을 고쳤다: 부제목·태그를 제목 아래로 내리고, 목록의 작성일은 짧은 형식(8/6 19:00)으로, 별은 23px 로. 넘침 0 확인.

**검증**: `test/regress-krjam-news.js` **111/111** — 기본 정렬 · 동점 시 최신 · 머리글 정렬 토글 · 단계/담당자/부서/검색 거르기 · 없을 때 안내 · 보이는 건수 · 목차의 협조부서와 취재 담당자.

### 16.105 v0.9.265 — 컵 참관단 일정표 이미지 기준 재구성 + 1회성 마이그레이션
- 사용자: 컵스카우트 프로그램 일정표(제공 이미지) 기준으로 완성. **1기 8/5(오후 입소)~8/7(오전 퇴소) · 2기 8/7(오후 입소)~8/9(오전 퇴소).**
- `cubObserverSeeds()` 전면 재작성(구 1기 8/4 시작 → 8/5): 등록·입소식(14:00)·과정활동·저녁식사·개영식(1기 8/5)·K-POP 콘서트(1기 8/6)·컵스나잇(2기 8/7)·폐영식(2기 8/8)·DMZ 견학(8/7·8/9 오전)·점심 12–13·아침 07–09·세면 및 취침 22–23. 총 **34건**.
- ⚠️ 시드는 **새 보드에만** 주입되므로(`mergeCubObservers` 의 `domainStored` 가드) 이미 저장된 라이브 보드는 자동 갱신 안 됨 → 신규 **`migrateCubSchedule()`**: 구 스케줄 신호(`cub-1-0804-*`)가 있을 때만 **1회성**으로 cub 시드-스킴 항목을 걷어내고 신 시드 주입(멱등·자체 가드, 사용자 생성 mkid cub 항목·타 트랙 보존). 로드 경로 `mergeCubObservers()` 다음에 배선. 클라 실행이라 사용자 세션으로 저장(운영 KV 직접 쓰기 없음). **구 8/4 항목의 담당 배정은 1회 초기화 → 새 일정에서 재지정 필요.**
- 검증: `node --check` + planning 회귀 **139/139 ×3라운드**(8/5 1기 5건·전체 34건·이미지 스케줄 어서션·마이그레이션[구 8/4 제거·신 시드 교체·사용자항목 보존·멱등] 신규, 옛 '컵스나잇 8/6' 하드코딩 dup 테스트를 새 슬롯 'K-POP 콘서트'로 갱신, 기존 flaky baseVer 테스트를 timetable-put 필터로 결정론화).

### 16.106 v0.9.266 — 참가자 일반식(crew_n) 이미지 기준 전면 교체 + 1회성 마이그레이션
- 사용자: "기존 일반식 메뉴 모두 삭제하고 이 이미지로 업데이트"(참가자 일반식). 표 열(8/5~8/9)·행(조·중·석) 더블체크.
- `defaultMeals().crew_n` 전면 재작성(이미지 대조로 기존 시드와 다른 값 교정: **롤유부초밥_햇반·요거톡**(8/6 조), **고기듬뿍김치찌개&라면사리**(8/6 석, 기존 '돼지김치찌개'), **클로렐라쌀밥**(8/5 석 2번째, 기존 '햇반'), **뽀로로두부봉**(8/7 중, 기존 '두부북'), **치킨마크니커리**(8/8 석, 기존 '마크네거리')). 8/5 조·중 없음(개영일 입영)·8/9 중·석 없음(귀가). 5일치.
- 시드는 새 보드에만 들어가므로(`upgradeMeals` 가드) 이미 저장된 보드는 신규 **`migrateMealsCrewN()`** 로 1회성 교체(자체 가드: 8/6 조식 '롤유부초밥_햇반' 표식 있으면 no-op·멱등). crew_s(특별식)·staff(운영요원)는 불변. 클라 저장(운영 KV 직접 쓰기 없음).
- 검증: `node --check` + planning **141/141 ×3라운드**(일반식 이미지 값 어서션·마이그레이션[구→신 교체·멱등] 신규, 기존 meals 7행×3 테스트 불변). cleanMeals(서버)가 crew_n 보존 확인.

### 16.107 v0.9.267 — 컵 참관단 일정 취재 담당 김승연 일괄 지정(1회성)
- 사용자: 홍보부 컵스카우트 일정 전체에 취재 담당자 = **김승연** 님 일단 일괄 지정.
- 신규 **`migrateCubReporterKSY()`**: 로드 시 명단에서 이름에 '김승연' 포함 인원을 찾아 **모든 cub(track='cub') 항목 assignees 에 추가**. 라이브 데이터 조치라 직접 KV 쓰기 불가 → 클라(사용자 세션)로 반영. **localStorage 로 브라우저당 1회만** 실행(이후 개별 해제를 매 로드마다 되살리지 않음 — 데이터 소실 패턴 회피). 명단에 '김승연' 없으면 no-op(플래그 미설정, 다음에 재시도). '일괄' 지시라 입영/오프타임 가드는 무시하고 전체 지정(입영 이후 도착이면 배치 화면에서 '입영 전' 충돌로 표시). `migrateCubSchedule()` 다음에 배선.
- ⚠️ **주의**: (1) 김승연 님이 홍보부 인원(roster) 명단에 있어야 적용됨. (2) 브라우저당 1회 — 여러 기기면 각 기기 첫 로드 때 서버에 이미 있는 값은 중복 안 됨(idempotent), 단 그 사이 개별 해제분은 다른 기기 로드 때 재추가될 수 있음(provisional '일단'). (3) 임시 조치라 이후 정리 시 이 마이그레이션 제거 가능.
- 검증: `node --check` + planning **142/142 ×3라운드**(김승연 전체 지정·1회 멱등 어서션 신규).

### 16.108 v0.9.268 — 잼버리 일정표에 '홍보부' 트랙(칼럼) 신설 + 일일 사진 셀렉·SNS 포스팅
- 사용자: 일정표에 홍보부 칼럼을 만들고 매일 반복 항목 추가. '오전/오후/저녁 브리핑'은 **페이스북 SNS 포스팅**(잼버리의 재미를 보여주는 콘텐츠)을 뜻함.
- **신규 트랙 'media'(홍보부)**: `TT_TRACKS`/`ttFilter`/`ttTrackOfItem`/필터 스와치/범례/일간뷰 열(`ttColumnHtml`)에 컵 트랙과 같은 패턴으로 추가. 트랙 색 `MEDIA_COLOR=#7A3FA0`(보라, `mediaColor()`=대비 보정) — 잼버리(초록)·의전(금)·컵(청/적갈)과 구분. 그룹라벨 `.gl-media`. 블록색은 `track==='media'` 면 mediaColor.
- **일일 시드**(`mediaTrackSeeds`, 8/5~8/9 × 6건 = 30건, 안정 id `media-MMDD-HHMM`): 11:30–12:30 1차 사진 셀렉 / 12:30–13:00 오전 브리핑·SNS 포스팅 / 16:30–17:30 2차 사진 셀렉 / 17:30–18:00 오후 브리핑·SNS 포스팅 / 22:00–22:30 3차 사진 셀렉 / 22:30–23:00 저녁 브리핑·SNS 포스팅. 브리핑 항목 memo=페이스북 SNS 포스팅 안내.
- **주입**(`mergeMediaTrack`): 신규 트랙이라 새 보드·기존 저장 보드 모두 대상, localStorage 로 브라우저당 1회(개별 삭제 되살림 방지)·id 멱등. 로드 체인 `migrateCubReporterKSY()` 다음 배선.
- ⚠️ 기간은 개영~폐영(8/5~8/9)로 시드. 사전일(8/2~8/4) 필요하면 `MEDIA_DAYS` 확장. 8/9 오후·저녁은 전체 오프(GLOBAL_OFF)와 겹치나 홍보부 항목 자체는 트랙 표시용(담당 배정만 가드됨).
- 검증: `node --check` + planning **142→145/145 ×3라운드**(트랙 시드 30건·항목·일간뷰 열/블록 렌더 신규, 기존 컵/의전 열 테스트 불변) + 헤드리스 스크린샷(4열: 잼버리·의전·홍보부[보라 6블록]·컵 참관단, 콘솔 0).

### 16.109 v0.9.269 — 홍보부 회의 → 홍보부 트랙 이동 + 편집 모달에 '홍보부' 트랙 옵션
- 사용자: 기존 '홍보부 일간 회의'를 홍보부 칼럼으로 이동. 대상 범위 = **홍보부 회의만**(홍보활동 종류는 그대로).
- 편집 모달 트랙 선택기에 **'홍보부'** 추가(기존 잼버리·컵1·컵2 옆) — 이제 어떤 일정이든 홍보부 트랙으로 직접 이동 가능(누락 보완). 핸들러에 `v==='media'` 분기.
- 신규 **`migrateMediaMoveMeetings()`**: 제목에 '홍보부' 포함 timetable 항목(홍보부 일간 회의 등)을 잼버리 열 → 홍보부 트랙으로 1회 이동(localStorage 브라우저당 1회, 컵·이미 홍보부 항목 제외, 멱등). 로드 체인 `mergeMediaTrack()` 다음 배선. 홍보활동 카테고리 항목은 이동 안 함(사용자 선택) — 필요하면 모달에서 개별 이동.
- 검증: `node --check` + planning **145→147/147 ×3라운드**(홍보부 회의 이동[프로그램 유지·멱등]·모달 트랙 옵션 신규).

### v0.9.282 — 잼버리 일정표 '시간 일정 편집' 모달 항목 순서 정리
- 사용자: 제목이 맨 위로, 이후 상식적인 순서로 정리.
- `renderTTModal` 필드 순서 재배치(핸들러는 id 로 바인딩 → 순서만 변경, 로직 불변):
  제목 → 종류 → 트랙 → 날짜 → 시간 → 반복 → 장소 → 현장 지도 구역 → 취재 불필요 → 담당 인원 → 관련 취재 연락처 → 메모 → 세부 일정(식순).
  (기존: 종류·트랙·날짜·시간·반복 다음에야 제목이 나와 파악이 어려웠음.)
- 검증: regress-krjam-planning **147/147 ×2** + nav. VERSION 0.9.282, planning html ?v 0.9.269→0.9.282.

### v0.9.286 — 부분 전멸 가드 + 도메인 되돌리기 (급식본부와 같은 조치)
- 사용자: "홍보부껏도 마찬가지로 작업해줘"(급식본부 데이터 유실 방지 작업에 이어).
- **지금까지의 보호가 못 보던 구멍**: 낙관적 잠금·병합(`conflictByOther`)은 "내가 불러온 뒤 **남이** 바꿨나"만 본다.
  혼자 편집하는 중에 항목이 대량으로 사라지는 저장 — 화면이 덜 그려진 상태에서 저장, 실수로 대량 삭제, 복원 실패 후 빈 배열 저장 —
  은 충돌이 아니라서 **그대로 통과했다**. v0.9.233 의 보드 백업(수동 JSON 내보내기)은 사람이 미리 받아 뒀을 때만 도움이 된다.
- **`saveDomain` 한 곳에 걸었다** — 15개 도메인 전부의 저장 길목이라 여기 한 번이면 전부 덮인다.
  호출부마다 if 를 붙이면 한 곳을 빠뜨리고 그 도메인만 무방비가 된다. 가드는 `GuardError` 를 던지고 `onRequestPut` 바깥에서 한 번 잡아 409 로 바꾼다.
  - 절반 미만으로 줄면 **409 `shrink_guard`** (항목 4개 이하는 가드하지 않음 — 확인이 습관이 되면 가드가 없는 것과 같다).
  - 확인하고 지웠으면(`confirmShrink`) **그때 직전 상태를 스냅샷**으로 남긴다.
    ⚠️ 매 저장마다 남기지 않는다 — 홍보부 보드는 디바운스 자동저장이라 KV 쓰기가 두 배가 된다. 정작 필요한 건 **사고가 난 그 직전 상태 하나**다.
  - 되돌리기: `GET ?snapshots=<도메인>` · `PUT {restoreDomain, at}` → 그 도메인만 복구(다른 도메인 무영향), 복구 직전 상태도 다시 스냅샷. `jp.restore` 감사 기록.
- **🔴 클라 쪽이 더 중요했다**: `sendDomainPut` 은 `if(!r.ok) throw` 라서 **409 를 네트워크 실패로 취급**한다.
  가드만 넣고 끝냈으면 대기 큐가 15초마다 영원히 재시도하고, 사용자는 이유도 모른 채 '저장 대기'만 봤을 것이다.
  → 409 를 별도로 받아 `onShrinkGuard()` 로: 무엇이 몇 개→몇 개로 줄어드는지 **사람이 읽는 이름**(DOMAIN_LABEL)으로 묻고,
  확인하면 `confirmShrink` 로 재전송, 취소하면 **`loadBoard()` 로 서버 값을 다시 불러온다**(화면이 '지워진 상태'로 남지 않게).
- 검증: `test/regress-krjam-planning-server.js` **16 → 34/34**. 실제 배포 모듈을 가짜 KV(Map)+실제 회원 세션으로 돌려
  10명→2명 저장이 409 로 멈추고 **아무것도 지워지지 않음**을 확인 → confirmShrink 로 저장 → 스냅샷 조회 → 되돌리기 → 감사 기록까지.
- 공용 모듈 `functions/api/_save-guard.js` 는 급식본부와 공유한다(→ [krjam-fnc/changelog.md](../krjam-fnc/changelog.md) v0.9.286).

### v0.9.287 — 기사 화면 느림(원인 2건) · 진행바 흔들림 · 식사 표 행↔열 반전
사용자: "기사 불러오는게 화가 날 정도로 오래걸리네" / "클릭하면 진행바 같은게 살짝 틀어진다" / "홍보부도 식사메뉴 행과 열을 바꾸자".

**① 🔴 목록을 KV 에서 한 건씩 순차로 읽고 있었다**
- `for (const k of res.keys) { await env.SCOUT_KV.get(k.name) }` — 레코드마다 왕복 1회를 **줄 세워** 기다렸다.
  기사가 N 건이면 N 번을 순서대로 기다린다. 쌓일수록 정직하게 느려지는 구조였다.
- **같은 패턴이 4곳**: 기사(jp-news) · 자료실(jp-assets) · 보도자료(jp-press) · 소식제보(jp-tips).
  홍보부 보드 GET(jamboree-plan)만 원래부터 `Promise.all` 이었고 나머지가 안 따라간 것이었다.
- 공용 `listRecords(env, prefix)` 신설(`_lib.js`) → 네 곳 모두 병렬. N 번 왕복이 사실상 1 번이 된다.
- ⚠️ **병렬로 읽으면 담기는 순서가 매번 다르다** → `byNewest` 로 정렬하고 createdAt 동점은 id 로 갈라 고정한다.
  안 그러면 새로고침마다 목록 순서가 뒤바뀐다(이번에 같이 넣은 검사가 그걸 지킨다).

**② 목록 응답에 옛 판 본문이 전부 실려 있었다**
- 기사는 수정할 때마다 그 판을 **본문·사진 통째로** history 에 쌓는다(최대 40판, 레코드 1건 상한 1.5MB).
  목록 API 가 그걸 전부 브라우저로 내려보내고 있었다 — 기사 수 × 판 수 만큼의 본문이 매번.
- 화면의 '버전 기록' 목록은 **v · 제목 · 작성자 · 시각**만 쓴다(`renderNewsHistory`). 본문은 옛 판을 실제로 열 때만 필요하다.
  → 목록에서는 history 의 본문·사진을 빼고, **`GET /api/jp-news?id=<id>`** 로 한 건만 통째로 받는다.
  클라 `ensureNewsDetail(id)` 가 기사당 한 번만 받아 채우고, 받는 동안은 '본문 불러오는 중…' 을 보여 준다
  (빈 문자열=본문 없던 판 과 구분한다 — 같은 문구면 "옛 판이 사라졌다"로 읽힌다).

**③ 진행바가 클릭할 때마다 흔들렸다 — 재현함**
- 콘텐츠 화면 라벨(`콘텐츠 완료 3/47 (6%) · 진행 시작 5 · 회의 9건`)은 카드를 누를 때마다 숫자가 바뀐다.
  9→10 처럼 **자릿수가 늘면** 좁은 화면에서 1줄 → 2줄로 접히고, 행이 커지면 `align-items:center` 가 바를 다시 가운데로 옮긴다.
- 실측: **w=430 에서 행 높이 +20.79px · 바 top +10.4px**. (w=390 은 짧은 라벨도 이미 2줄이라 델타가 0 → 폭을 훑지 않으면 못 잡는다.)
- 좁은 화면(≤680px)에서 바를 제 줄에 올려(`flex-direction:column`) 라벨이 몇 줄이 되든 바 위치가 고정되게 했다.
  360·390·430·600·680·768·1024·1440 **여덟 폭 전부 이동 0** 으로 회귀에 고정(motion 25→33).

**④ 잼버리식당 식사 메뉴 행↔열 반전**
- 행=조식·중식·석식, 열=날짜. 급식보드(/krjam-fnc)가 v0.9.271 에 이 방향으로 갔는데 홍보부만 반대였다.
  **같은 자료(`jp:meals`)를 두 화면이 서로 다른 방향으로 보여 주면** 옮겨 적을 때 실수가 난다. 한 끼를 일주일치로 나란히 보는 쪽이 식단 비교에도 낫다.
- 머리행(날짜)은 `renderMeals` 가 그린다(`#mealhead`). 칸마다 `data-d`(날짜)·`data-c`(끼니)를 둬서 저장 경로가 방향에 의존하지 않게 했다.
- 모바일 카드 레이아웃에서는 **카드 하나 = 끼니 하나**, 그 안에 날짜별 칸. 행 머리를 카드 제목처럼 세웠다.

- 검증: planning **149→152** · planning-server **34→53** · motion **25→33** · news **111→112**,
...
### 16.xx v0.9.290 — 잼버리 일정표 '입·퇴영' 호차 트랙 신설 (입영+퇴영 한 열)
대상: `/krjam-planning`(`jamboree-plan/app.js`·`styles.css`, `functions/api/jamboree-plan.js`). 사용자가 참가단 버스 호차의 입영(8/3~8/5)·퇴영(8/9) 일정을 주며 **"별도의 열을 만들어서, 입영과 퇴영을 한 열로"** 관리 요청.

- **왜 새 트랙인가**: 일정표에는 이미 `잼버리 일정 · 의전 · 홍보부 · 컵 1기 · 컵 2기` 트랙이 있고 일간 뷰에서 각각 별도 열로 그려진다. 호차 일정을 카테고리(`ttcats`)로 넣으면 잼버리 열 안에 섞여 프로그램 일정을 밀어낸다 → **컵·홍보부와 같은 독립 트랙**(`track='bus'`)이 요청한 "별도의 열"과 정확히 같은 구조다.
- **입영·퇴영을 한 열에 둔 이유**(사용자 지시): 트랙을 둘로 쪼개면 같은 호차의 왕복을 나란히 못 본다. 트랙은 하나만 두고 **방향은 새 필드 `dir('in'|'out')`** 으로 들고, 블록 안 태그(`입영`/`퇴영`)로 구분한다. 편집 모달의 트랙 버튼도 `입영 (입·퇴영)` / `퇴영 (입·퇴영)` 둘이지만 **가는 열은 하나**다.
- **⚠️ `dir` 은 화이트리스트 두 곳에 넣었다** — 클라 `buildCleanTT` + 서버 `cleanTT`. 하나라도 빠지면 **저장할 때마다 조용히 유실**되어 입영/퇴영 구분이 통째로 사라진다(`roster.arrive` 가 v0.9.223→229 에 실제로 그랬다). 서버 `cleanTT` 를 `export` 로 열어 순수함수 회귀 6건으로 못 박았다.
- **시드 16건**(입영 9 · 퇴영 7), 안정 id `bus-<in|out>-MMDD-HHMM-<호차 slug>`. 주입은 `localStorage` 로 **브라우저당 1회**(사용자가 지운 호차를 매 로드마다 되살리지 않는다). **track/dir 복구 루프는 플래그와 무관하게 매번 돈다** — id 가 스스로 트랙·방향을 증명하므로, 편집으로 값이 벗겨져도 되살아난다(컵 트랙이 v0.9.195 이전에 겪은 `track` 유실을 미리 막는다).
- **블록 길이 30분 고정**(사용자 확정) — 원문에 출발·도착 '시각'만 있어 종료 시각이 없다. 드래그로 언제든 조절된다.
- **기본 열 비중 `bus:2`**(다른 트랙은 1): 8/9 퇴영이 09:00~09:50 사이 **10분 간격으로 5대**라 30분 블록이 전부 겹쳐 그 열만 **레인 3개**로 갈라진다. 비중 1이면 호차명도 안 들어간다. `ttColWDefaults()` 로 기본값을 한곳에 모으고 더블클릭 리셋도 여기서 가져간다(토스트 문구 '균등' → '기본값').
- **부수 처리**: 보기 필터 칩·범례 스와치 추가 / 촬영 리스트에서 제외(호차는 취재 대상이 아님) / 전체기간 뷰는 기존 트랙과 같이 통합 레인.
- **색**: `#9B3B57`(자홍). 잼버리 초록·의전 금·컵 청/적갈·홍보부 보라와 색상환에서 떨어뜨렸다. 흰 글씨 대비 6.65:1 로 `darkenToContrast` 가 손대지 않는다. 열 라벨은 흰 배경 위라 더 진한 `#8E2F4B`(7.9:1).
- **원문 표기 정정 3건**(사용자 확정): ⓐ 8/9 C호차 `우크라이타` → **우크라이나** ⓑ 8/4 B호차 `대만, 슬로베니아, 홍콩, 대만` → **대만 중복 1건 제거** ⓒ 8/5 `12호차` 는 **원문 그대로 유지**(앞 호차가 A~I 라 J호차 여부 확인 필요 — 해당 항목 메모에 남김). ⓐⓑ 도 각 항목 메모에 원문을 적어 되돌릴 수 있게 했다.
- 검증: planning **154→175** · planning-server **53→59**(전체 **804→831건**). 17종 **2라운드 ALL GREEN**. 라이브 재검증: VERSION 0.9.290 · `/krjam-planning` 200 · app.js·styles.css·krjam-planning.html 로컬=라이브 SHA-256 일치. `?v=` app.js 0.9.287→0.9.290 · styles.css 0.9.289→0.9.290.
- ⚠️ **삽질 메모**: 배포 직후 `curl -sL https://scoutingapp.net/krjam-planning` 이 **옛 `?v=` 를 물고 왔다**(엣지 캐시). 같은 호출의 SHA-256 은 새 것과 일치 — 즉 **호출마다 다른 POP** 을 맞았다. 라이브 재검증은 `Cache-Control: no-cache` + 쿼리 캐시버스터로 **여러 번** 확인해야 한다. `/VERSION` 만 보고 판단하면 자산 캐시를 놓친다.

### 16.xx v0.9.291 — 일정표 현재 시각 인디케이터 + 대시보드 '지금 현장'(진행 중 · 3시간 내, 열 단위)
대상: `/krjam-planning`(`jamboree-plan/app.js`·`styles.css`, `krjam-planning.html`). 사용자 요청: 일정표에 **실시간 빨간 선**으로 현재 시각을, 대시보드에는 **지금 진행 중인 일정**과 **앞으로 3시간 이내에 열릴 행사**를 **열 단위로 구분**해서.

**① 일정표 현재 시각 인디케이터**
- **오늘 날짜 열에만** 빨간 가로선 + 왼쪽 `HH:MM` 배지(`.ttg-now`). 그리드가 8/2~8/9 라 **오늘이 그 밖이면 아무것도 그리지 않는다**(현재 2026-07-31 기준 안 보이는 것이 정상).
- **30초마다 선의 `top` 만 옮긴다.** 그리드를 통째로 다시 그리면 드래그 중인 블록이 튀고 포인터 캡처가 끊긴다(열 너비 드래그에서 이미 겪은 문제). 자정을 넘겨 **날짜가 바뀔 때만** `renderTimetable()` 로 선을 다른 열에 옮긴다.
- **계산과 그리기를 갈랐다** — `ttNowGeometry(day, today, nowH, hh)` 는 인자만 보는 순수함수다. 실제 시계에 기대면 회귀에서 검증할 수 없어, 테스트가 `todayISO`·`nowHours` 를 갈아끼워 8/5 14:30 을 고정 시나리오로 못 박는다.
- ⚠️ **현재 시각은 내림(`h2hhmmFloor`)** — 기존 `h2hhmm` 은 **반올림**이라 14:29:31 을 `14:30` 으로 보여 준다(시계가 30초 앞선다). 15분 스냅되는 일정 시각과 달리 '지금'은 내림이 맞다. 회귀에 반올림 결과와 나란히 비교해 고정.
- `z-index:5`(블록 1 위·구분선 6 아래) + `pointer-events:none` — 선이 블록 클릭을 먹지 않는다. 화면을 떠나면 `setView` 가 타이머를 끈다(`document.hidden` 이면 tick 도 건너뜀).

**② 대시보드 '지금 현장' — '오늘의 현장' 패널 안**
- `진행 중` / `앞으로 3시간` 두 구획을, 각각 **일정표의 열(트랙) 단위로 묶어** 보여 준다(사용자 확정). 열 순서는 `TT_TRACKS` 와 같다 — 잼버리 → 의전 → 홍보부 → 컵 1·2기 → 입·퇴영. 항목마다 `N분 남음` / `N분 뒤`.
- **의전도 한 열로 들어온다.** 일정표 열이 쓰던 '같은 시각·장소 = 한 행사' 묶음 로직을 `protPseudoEvents(day)` 로 빼서 **두 화면이 같은 함수를 쓰게** 했다(복제하면 두 화면이 어긋난다).
- ⚠️ **보기 필터(`ttFilter`)를 일부러 적용하지 않는다.** 일정표 화면에서 잠깐 꺼 둔 열 때문에 현장 상황이 빠지면 안 된다 — 회귀로 고정.
- **갱신은 분 단위**(D-day 카운트다운은 그대로 1초). 1초마다 목록을 갈아끼우면 누르려던 항목이 손끝에서 사라진다(클릭 유실). `dashNowMin` 으로 분이 바뀔 때만 다시 그리고, 그 블록의 클릭은 `renderDashNow` 가 스스로 다시 건다.
- 경계 규칙: **끝나는 순간은 진행 중이 아니다**(14:00~14:30 호차는 14:30 에 빠짐). '3시간 내'는 창 끝에 **정확히 걸치면 포함**. 둘 다 회귀에 있다.
- 잼버리 기간 밖이면 안내 문구로 대체하고, 기간 **전/종료**를 구분해 문구를 고른다. 기존 '오늘 전체' 목록은 그대로 두고 소제목만 붙였다.
- 부수 정리: 트랙 색 계산이 필터 칩과 여기 두 곳에 복제돼 있어 `ttTrackColor(key)` 하나로 합쳤다.
- 검증: planning **175→200**(전체 **831→856건**). 17종 **2라운드 ALL GREEN**. 라이브 재검증: VERSION 0.9.291 · 라우트 200 · app.js·styles.css·krjam-planning.html 로컬=라이브 SHA-256 일치. `?v=` app.js·styles.css 0.9.290→0.9.291.
- ⚠️ **전파 교훈(v0.9.290 에 이어 재확인)**: `/VERSION` 이 0.9.291 로 바뀐 **뒤에도** 자산은 POP 마다 옛 바이트를 1분 남짓 더 준다. 이번엔 `Cache-Control: no-cache` + 캐시버스터로 **5회 연속 전 자산 일치**가 될 때까지 `until` 로 돌려서 확인했다. 1회 확인은 통과처럼 보여도 근거가 안 된다.

### 16.xx v0.9.292 — 야영장 배치도 2026-07-23 최종본 반영 + 현장 지도 구역 재정비
대상: `/krjam-planning`(`jamboree-plan/app.js`·`assets/sitemap.png`, `krjam-planning.html`) + `functions/api/jamboree-plan.js` + `/krjam-jebo`. 사용자: *"이거 지도 최종본이야 업데이트해줘"*(시설물자관리본부 2026-07-23 판).

**① 이미지만 갈아끼우면 안 되는 이유**
- 핀 좌표는 배치도 이미지 **2000×1414 기준 비율(0~1)** 이다. 배치가 바뀐 자리에 옛 좌표를 그대로 두면 **핀이 엉뚱한 건물을 가리킨다** — 그런데 화면은 멀쩡해 보여서 아무도 눈치채지 못한다.
- 그래서 **구역 좌표를 눈대중이 아니라 실측**했다: 원본을 BMP 로 바꿔 색상 블롭(과정 파랑 `#004FFF` · 급식 하늘 `#00C3FF` · 안전 노랑 `#FCDF00` · 분단영지 연노랑 `#FFFF7B` 등)의 연결 성분 무게중심을 뽑아 좌표를 잡았다. 분단 영지 7곳·JHQ·전시관·물자보급은 **위치가 그대로**임을 이 방법으로 확인하고 손대지 않았다(불필요한 좌표 드리프트 금지).

**② 바뀐 것 (6/25 판 → 7/23 판)**
- **신설 4**: `p5` 과정5 활동장(우측 `0.606,0.568`) · `p7` 과정7 활동장(좌하단 `0.080,0.919`) · `safety` 안전본부(`0.171,0.547`) · `park` 주차장(`0.489,0.252`).
- **폐지 2**: `staffpark`(운영요원 주차장) · `buspark`(버스 주차장) — 신 배치도에서 두 구분이 사라지고 안전본부 색 대형 영역 안 **P 한 곳**으로 통합됐다. 사용자 확정으로 `park` 하나로 교체하고, 옛 장소명 `버스 주차장`·`운영요원주차` 는 **별칭으로 흡수**해 기존 일정의 자동 매칭이 끊기지 않게 했다.
- **라벨 정정 3**: `메인무대`→**대집회장** · `급식`→**급식편의본부** · `과정5·체육관`→**과정6 활동장·체육관**. 옛 장소명(`메인무대`·`급식`)은 별칭으로 남겨 **기존 일정·의전 데이터의 매칭을 유지**한다.
- 과정6 과 체육관은 배치도상 **같은 파란 건물에 라벨이 둘**이라 한 구역으로 묶었다(사용자 확정). 나눴다면 핀 2개가 완전히 같은 자리에 겹친다 → 회귀에 '같은 자리 구역 없음' 검사를 넣어 못 박았다.
- 자산: `sitemap.png` 2000×1414 재생성 + 고해상 원본 `KakaoTalk_Photo_2026-08-02-00-13-17.png`(3508×2481) 보관(`_middleware` 가 공개 차단, 404 확인). **`img src` 에 `?v=` 캐시버스터를 붙였다** — 없으면 브라우저·엣지가 옛 지도를 계속 준다(이미지는 파일명이 같다).

**③ 파묻혀 있던 진짜 버그 — `본부` 별칭이 너무 넓었다**
- `jhq` 의 별칭에 `'본부'` 가 있었다. `zoneForPlace` 는 부분문자열 매칭이라 **`안전본부`·`급식편의본부` 가 전부 JHQ 로 빨려 들어갔다.** 신설 구역을 넣고 회귀를 돌린 순간 `안전본부→jhq` 로 드러났다.
- v0.9.212 의 **`'무대'` 별칭이 `소무대` 를 메인무대로 끌어갔던 것과 같은 함정**이다. 별칭에서 `'본부'` 만 뺐다 — 맨 `본부` 는 **라벨 `JHQ 본부` 가 그대로 잡아 주므로** 매칭을 잃지 않는다(회귀 2건으로 양쪽 다 고정).

**④ 분단 연맹 목록 = 배치도 기준으로 1회 정합** (사용자 확정: 지도가 정본)
- 지워진 것 4: 큰물결·평화숲의 **말레이시아**, 푸른별의 **대구연맹·라이베리아**, 꿈동산의 **홍콩**.
- 시드만 고치면 **이미 저장된 보드에는 반영되지 않는다**(`state.divisions` 는 KV 저장 대상). `migrateDivisionFeds()` 로 1회 정합하되:
  - ⚠️ **멱등 가드를 '지도에 없는 연맹을 지운다'로 만들면 안 된다** — 사용자가 나중에 직접 더한 연맹까지 매 로드마다 다시 지운다(조용한 데이터 유실). 분단마다 **`fedver='2026-07-23'` 표식**을 찍고, 찍힌 분단은 두 번 다시 건드리지 않는다.
  - ⚠️ **`fedver` 는 서버 `cleanDivision` 화이트리스트에도 넣었다.** 빠지면 저장 때마다 표식이 날아가 같은 마이그레이션이 **매 로드 반복** — `roster.arrive`(v0.9.229)·`tt.dir`(v0.9.290) 과 같은 함정. `cleanDivision` 을 `export` 해 순수함수 회귀 3건으로 고정.
  - 저장된 적 없는 보드(`domainStored('divisions')===false`)에서는 **아무것도 하지 않는다** — 시드를 되살려 저장하면 사용자가 지운 분단이 부활한다(v0.9.232 교훈).
  - 분단장·운영·안전·지원 칸과 사용자가 직접 추가한 분단은 손대지 않는다.

**⑤ 회귀가 잡아낸 테스트 자체의 결함** (DoD 2-1)
- `잼버리 기간 밖이면 안내 문구를 보여 준다` 가 **실제 시계**로 재고 있었다. 오늘(2026-08-02)이 잼버리 기간(8/2~8/9)에 들어서면서 **FAIL** — 즉 정작 **행사 기간 8일 내내 스위트가 빨개질** 참이었다. `todayISO` 를 `2026-07-01` 로 밀어 넣어 재현하도록 고쳤다.

**검증**: planning **200→225** · planning-server **59→62** · jebo **29→32** → 17종 **856→887건**, **2라운드 ALL GREEN** + 감사 2종 0건.
`/krjam-jebo` 의 구역 목록도 같은 key 로 동기화하고, 개수를 박아 둔 검사(`구역 30개`)를 **`ZONES.length` 기준 + key 단언**으로 바꿨다(배치도가 바뀔 때마다 숫자 때문에 걸리던 것).
라이브 재검증: VERSION 0.9.292 **6회 연속 일치** · 전 라우트 200 · `sitemap.png`·`app.js`·`krjam-planning.html`·`krjam-jebo.html` 로컬=라이브 SHA-256 **5회 연속 일치** · 고해상 원본 2종 404(공개 차단 유지).

### 16.xx v0.9.293 — 시스템 안정성 종합점검: 보드 GET 병렬화 + 구역 정합 고정
사용자: "시스템 안정성 종합검토" (대상=jimmyport 전 영역). 아래는 홍보부 몫이고, 급식본부·D-Count 는 각 changelog 참조.

**① 🔴 보드 GET 이 KV 를 15번 순차로 읽고 있었다** (`functions/api/jamboree-plan.js`)
- `onRequestGet` 이 14개 도메인 키 + roster 를 `const x = await rd(...)` 로 **한 줄씩 줄 세워** 읽었다. KV get 은 매번 왕복이라, 보드를 한 번 열 때마다 왕복 15번을 순서대로 기다린 것이다.
- v0.9.287 이 목록 4종(기사·자료실·보도자료·제보)의 같은 패턴을 `listRecords` 로 고쳤는데, **정작 가장 자주 열리는 보드 GET 본체가 안 따라와 있었다.** 위쪽 슬롯 루프만 `Promise.all` 이라 "보드는 원래 병렬"로 보였던 것이 함정 — handoff 에도 그렇게 적혀 있었다.
- 엣지 캐시는 30초 TTL 이고 **저장할 때마다 purge** 되므로, 편집 중에는 사실상 매 로드가 이 경로다.
- 조치: 15개를 `Promise.all` 한 묶음으로. roster 는 teams 를 같이 꺼내야 해서 별도 함수로 감싸 같은 묶음에 넣었다.
- ⚠️ 병렬이면 `versions[]` 에 담기는 순서가 매번 달라진다 — 키로 쓰므로 순서 의존이 없고, payload 필드 순서는 객체 리터럴이 고정한다.
- 회귀: **시간(ms)이 아니라 동시에 떠 있던 get 개수**로 잰다(v0.9.287 방식). 순차로 되돌리면 `동시 get 최대 1` 로 FAIL 하는 것까지 확인 — 병렬이면 14.

**② 구역 목록(ZONES) 정합을 두 파일 맞대어 고정**
- `ZONES` 는 `jamboree-plan/app.js`(핀 좌표)와 `krjam-jebo.html`(제보 드롭다운)에 **같은 key 로 복제**돼 있다. 한쪽만 고치면 제보의 `zone` 이 보드에서 미아가 되는데 **화면은 양쪽 다 멀쩡해 보인다**.
- 지금까지 두 스위트가 각자 '기대값'(p5·safety 있음 등)만 따로 확인했다 → **한쪽에만 구역을 더하거나 라벨을 바꾸면 두 스위트가 모두 통과한다.** 실제 상태는 일치했지만 **강제하는 것이 아무것도 없었다.**
- 조치: 두 파일을 직접 읽어 key 집합·개수·한글 라벨·순서를 맞대는 검사 6건을 순수함수 스위트(브라우저 불필요)에 추가. 라벨 1개 변경 + 구역 1개 삭제를 주입해 4건이 FAIL 하는 것으로 이빨 확인.
- ⚠️ 파일 경로는 CWD 가 아니라 **`import.meta.url` 기준**으로 찾는다 — CWD 에 기대면 다른 데서 돌릴 때 조용히 빈 값이 되어 늘 통과한다.

**검증**: planning-server **62→85** → 17종 **887→914건**, 2라운드 ALL GREEN + 감사 2종 0건.

### 16.xx v0.9.294 — 개영식 기준 카운트다운을 전 사이트 공용으로 (8/9 12:00 이후 숨김)
사용자 확정(2026-08-04): **행사의 공식 시작은 개영식 2026-08-05 20:00** — 카운트다운은 그걸 기준으로 한다. 개영식이 지난 뒤에는 `D-DAY · 개영!` 을 유지하고, **2026-08-09 12:00 이후에는 아예 보여 주지 않는다.** 적용 대상 4곳: 랜딩 `/` · 홍보부 `/krjam-planning` · 급식본부 `/krjam-fnc` · 소식제보 `/krjam-jebo`(D-Count 는 제외).

**① 공용 모듈 `/countdown.js` 신설**
- 같은 계산이 네 화면에 필요하다. 복사해 두면 개영 시각이 바뀔 때 **세 곳만 고치는 사고**가 난다(CLAUDE.md: 스파게티 복제 금지 · 신규는 별도 모듈로). `window.KJCountdown` 하나만 고치면 전부 따라온다.
- `state(now)` 는 **시계를 읽지 않고 인자만 본다** — 안에서 `new Date()` 를 부르면 회귀가 실제 시각에 끌려가 날짜가 지나는 순간 스위트가 통째로 빨개진다(v0.9.292 · v0.9.293 실제 사고 2건). 화면에 그릴 때만 `mount()` 가 시계를 넣는다.
- 🔴 **기준 시각을 KST 고정 오프셋(+09:00)으로 못 박았다.** 예전 홍보부는 `new Date(2026,7,5,20,0,0)` — **브라우저 타임존 기준**이라 해외에서 보면 엉뚱한 시각을 셌다(제보 화면은 영문도 지원한다 = 해외 참가자가 본다). ⚠️ **이 버그는 KST 기기에서는 절대 안 보인다** — 그래서 회귀는 UTC 환산값이 아니라 **`OPEN_ISO` 문자열 자체**를 검사한다(기기 타임존과 무관하게 걸린다).
- 하루 미만이면 `D-0` 을 붙이지 않는다 — `D-0 00:00:01` 은 고장 난 것처럼 보인다. 옆에 '개영식까지' 라벨이 있어 시:분:초만으로 뜻이 통한다.

**② 홍보부 — 공용 모듈로 위임 + 종료 후 숨김**
- `EVENT_DT`·`ddayCountdownHTML()` 의 자체 계산을 걷어내고 `cdState(now)` 로 위임. 상단바 시계(`#m-clock`)와 대시보드 배너(`.dash-dday`) 둘 다 대상.
- 8/9 12:00 이후: 배너는 **아예 그리지 않고**(아래 내용이 위로 올라온다), 상단 시계는 `.clock` 묶음째 숨긴다. 예전엔 개영식이 지나도 `D-DAY · 개영!` 이 행사 끝난 뒤까지 대시보드 맨 위를 차지했다.
- 🔴 **하마터면 만들 뻔한 회귀**: `'지금 현장'` 분 단위 갱신이 **카운트다운 타이머에 얹혀** 있었다(`dash-dday-t` 가 없으면 `stopDashClock()`). 배너를 숨기자 현장 목록 갱신까지 같이 죽었다 → 두 일을 떼어 놨다. 회귀로 고정했고, 옛 결합으로 되돌리면 FAIL 하는 것까지 확인했다.

**③ 랜딩 · 제보 — 신규 추가**
- 랜딩: 히어로 아래 pill 하나. 제보: 히어로 안, **ko/en 문구 모두**(`cdLab`·`cdOpen`).
- ⚠️ 제보는 언어를 바꿀 때마다 `mount` 를 다시 걸면 **1초 타이머가 쌓인다**(초가 건너뛴다) → 새로 걸기 전에 항상 옛 타이머를 끈다. 언어 8회 전환 후 3초에 2~4초만 줄어드는지로 회귀 고정.
- 마크업은 `hidden` 으로 시작한다 — 모듈 로드 실패나 행사 종료 시 **빈 칸이 잠깐 보였다 사라지는 깜빡임**을 만들지 않는다.

**검증**: planning-server **85→104**(경계값이 여기가 정본) · planning **225→234** · 랜딩 **38→45** · 제보 **32→40** · fnc보드 **84→86**.
이빨 확인 4종: 종료 시각 무시 → server 5건 + 랜딩 1건 FAIL · 타임존 로컬 복귀 → FAIL · `D-0` 복귀 → 2건 FAIL · 타이머 재결합 → FAIL.
전체: 17종 **914→959건**, **2라운드 ALL GREEN**(두 라운드 결과 완전 동일) + 감사 2종 0건.

### 16.xx v0.9.295 — 3개 서비스 공식 종료(카드뉴스 제작기 · 디데이 프로젝트 · 급식편의본부)
사용자 지시(2026-08-14): *"카드뉴스 제작기와 디데이 프로젝트 그리고 급식본부는 한곳에 모아서 알집으로 모아놔줘. 모아놓으면 내가 한곳에 별도로 보관할 꺼고, 서비스는 공식적으로 그만할꺼야."* + 작업 중 추가 지시 *"디데이 프로젝트 사진들은 삭제하면 안된다잉"*.
사용자 확정 3건(AskUserQuestion): **종료 안내 페이지**로 대체(404 아님) · **홍보부 연동도 같이 걷어냄** · **문서까지 전부 삭제**.

**① 먼저 아카이브 — 지우기 전에 되찾을 수 있게**
- `KRJAM16-종료서비스-아카이브-20260814.zip`(74MB · 파일 160개, 사용자 보관). 코드·정적자산·서버 API·changelog + **라이브 데이터 스냅샷**.
- 🔴 **디데이 사진 29장은 저장소에 없었다** — 라이브 KV(`img:<id>`)에만 있었고 `dcount:app:*` 이 URL 로만 참조했다. **코드만 백업했으면 그대로 잃을 뻔했다.** 승인 26건을 훑어 전부 내려받았다(`D-23_2026-07-13_신나명_2.jpg` 형식 + `사진-목록.json` 이 신청자·소속·문구와 이어 준다).
- 함께 받은 것: 카드뉴스 저장본 2건 · 급식 조직표/명단(전화 마스킹본)/구역배정/문구/식사메뉴.
- 라이브에는 **GET 만** 했다(운영 KV 파괴적 쓰기 금지 규칙). 압축 해제본을 원본과 `diff -r` 해 차이 0, 사진 29장 헤더·PDF `%%EOF` 까지 확인.

**② 홍보부에서 걷어낸 6군데** — 이게 이번 작업의 실제 위험이었다
캘린더 자동 연동(`fetch('/api/krjam-dcount')` + 승인 카드 렌더·사진 썸네일·클릭 핸들러) · 대시보드 '디데이 프로젝트' 패널 + 통계 카드 · 사진 첨부창의 '카드뉴스 제작기 열기' · 기사 '카드뉴스 만들기'(제작기로 넘기기) · 인원 R&R 시드 문구 · 관련 CSS 4블록.
- ⚠️ **`cardnewsDone`('카드뉴스 가공됨') 플래그는 남겼다** — 이름만 같을 뿐 제작기와 무관한 **홍보부 업무 진행 표시**다. 같이 지웠으면 기사마다의 진행 상태를 잃는다.
- ⚠️ **콘텐츠 유형 `'dcount'`(D-count 카드뉴스 종류)도 남겼다** — 서비스가 아니라 콘텐츠 분류다. 그래서 종료 회귀는 문자열 `dcount` 가 아니라 **따옴표로 감싼 링크 형태**로만 잰다.

**③ 지운 것 / 남긴 것 — 경계가 함정이었다**
- 🔴 **`jamboree/` 를 통째로 지우면 안 된다.** 카드뉴스 모듈 폴더처럼 보이지만 `assets/logo.png`(엠블럼)·`og-planning.png` 를 **랜딩·홍보부·제보가 아직 쓴다.** 통째로 지웠으면 아이콘이 전부 사라지는데 **화면은 그대로 떠서 눈으로는 안 보인다.** → `*.jsx`·`store.js`·`shapes.js`·`fonts/` 와 종료 서비스 전용 이미지 4개만 지우고 공유 자산은 남겼다.
- 남긴 것: `jp-meals`(급식↔홍보부 공유 메뉴) · `image.js`(제보도 쓴다) · `_lib.js` 의 `fncAdmin`(`jp-meals` 가 아직 import 한다 — 살아 있는 서비스는 건드리지 않는다).
- ⚠️ **라이브 KV 는 하나도 지우지 않았다**(사용자 지시). API 만 없앴다 — 값은 `jamboree*`·`dcount:*`·`img:*`·`jp-fnc*` 그대로 남아 있다.

**④ 종료 안내 화면 `/service-ended` (신규)**
- 인쇄물·QR·SNS 에 뿌려진 주소라 404 로 두지 않았다. `_redirects` 가 5개 경로를 `?s=<키>` 로 넘기고 **화면 하나가 이름·설명만 갈아 끼운다**(3개 화면을 복제하지 않는다).
- ⚠️ **301 이 아니라 302.** 301 은 브라우저가 영구 캐시해서, 나중에 되살려도 이용자가 **안 돌아온다.**
- ⚠️ 키가 없거나 모르는 키여도 화면이 성립해야 한다(기본 문구를 HTML 에 박아 두고 JS 는 덮어쓰기만).
- 디데이 안내에는 **사진·신청 정보를 보관 중**이라고 적었다 — 사진을 올린 참가자가 가장 먼저 궁금해할 것이다.

**⑤ 회귀 — 오늘 이미 빨간 상태였다(내 변경과 무관한 결함 4스위트 13건)**
🔴 개영식 카운트다운은 **8/9 12:00 이후 숨기는 게 정상**인데, 랜딩·제보·홍보부의 '그려지는가' 검사가 **실제 시계로** 재고 있었다. 오늘(8/14)이 그 시각을 지나 **행사 뒤로는 영원히 FAIL** 날 참이었다 — v0.9.292·293 에서 두 번 걸린 **바로 그 함정**이 세 번째로 재발했다.
- 고침: 카운트다운이 떠 있는 시각(2026-08-04 20:00 KST)으로 **갈아끼워** 잰다. ⚠️ **얼리면 안 된다** — '초 단위로 흐른다'·'초가 건너뛰지 않는다'를 같이 재기 때문에 **고정 시각에서 출발해 실제 속도로 흐르는** 시계를 넣었다.
- 랜딩 '진입 연출 후 전부 보임'도 같은 뿌리 — 숨긴 카운트다운 박스의 opacity 가 0 인 게 정상인데 필터가 없었다. **보이는 것만** 재도록 고침.
- 교훈 재확인: `state(now)` 를 순수함수로 만든 것만으로는 부족하다. **화면 검사도 시각을 인자처럼 갈아끼워야 한다.**

**⑥ 신규 `test/regress-service-ended.js` (25건)** — 종료를 **양쪽에서** 막는다
- **덜 지움**: 랜딩·홍보부·제보·운영현황에 종료 주소로 가는 길이 남았는가(6군데였다) · 유입 집계 화이트리스트.
- **더 지움**: 공유 자산이 살아 있는가 — 파일 존재 + **실제 렌더(`naturalWidth>0`)** 둘 다. 파일만 보면 경로가 어긋나도 통과한다.
- `_redirects` 302 여부 · `?s=` 키별 문구 · 무키/오키 fallback · 접근성 실측(13px·40px·모바일 390 넘침 0).
- **이빨 확인 4종**: 공유 자산 삭제 → 3건 FAIL · 302→301 → 2건 FAIL · 삭제 파일 부활 → FAIL · 디데이 보관 문구 제거 → FAIL.

**검증**: 삭제 스위트 4종(fnc 3 + fnc감사) 제외, 신규 1종 추가 → 14종. planning **234** · server **104→94**(디데이 서버 검사 10건은 대상 코드와 함께 제거) · news **112→113** · 랜딩 **45→46** · 종료 **25** = **728건**, **2라운드 ALL GREEN** + 감사 1종 0건.

### 16.xx v0.9.296 — 지운 경로가 실제로 404 가 되게 (`404.html` 신설)
v0.9.295 배포 **직후 라이브 실측**에서 잡았다.

- 🔴 지운 자산·API 가 **404 가 아니라 랜딩 HTML 200** 이었다 — `/api/krjam-dcount`·`/jamboree/app.jsx`·`/krjam-fnc/pages/p01.webp` 가 전부 `<title>Scouting App — 도구 모음</title>` 을 200 으로 돌려줬다.
- **이번 삭제 탓이 아니다.** `/nonexistent-xyz-123` 도 같은 결과였다 — Cloudflare Pages 는 `404.html` 이 없으면 **못 찾은 경로에 `index.html` 을 200 으로** 준다(원래 동작). 배포 검증에서 이걸 '삭제 실패'로 오진하면 엉뚱한 곳을 판다.
- 그래도 고쳐야 한다: (ㄱ) 지웠는데 안 지워진 것처럼 보이고 (ㄴ) 옛 클라이언트가 `r.json()` 에서 HTML 을 받고 (ㄷ) 검색엔진이 없는 페이지를 200 으로 색인한다.
- ⚠️ **끄기 전에 안전부터 확인했다** — catch-all 을 없애면 경로 기반 클라이언트 라우팅이 있는 사이트는 딥링크가 통째로 깨진다. 이 저장소는 `pushState` 가 admin 의 **해시** 하나뿐이라 안전하다(실측 후 진행).
- 404 화면은 종료 안내와 같은 디자인 언어(단일 카드·엠블럼·40px 버튼)로, 도구 모음으로 돌아가는 길 하나만 준다.

**검증**: `regress-service-ended` 25→**28**(404.html 존재·복귀 링크·공유 엠블럼) · a11y 스윕이 `/404` 도 훑는다(24→**28**) → 14종 **728→735건**, 2라운드 ALL GREEN.
