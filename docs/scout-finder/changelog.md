...
### v0.9.300 — `/tour` 파비콘 404 제거 + 실패 기록 문서 신설
**① 파비콘** — `/tour`·`/tour/admin` 에 파비콘 선언이 아예 없어, 브라우저가 알아서 `/favicon.ico` 를 요청하고 **매번 404** 가 났다(v0.9.299 라이브 점검에서 잡힘 · 폴더 이동과 무관한 기존 결함).
- ⚠️ **잼버리 엠블럼(`/assets/logo.png`)을 붙이지 않았다.** 그건 제16회 한국잼버리 행사 엠블럼이고, `/tour` 는 **영문 Scout Tour Assistant** 다 — 뜻이 어긋난다. `/admin` 이 이미 쓰던 **인라인 SVG 브랜드 마크**(보라 라운드 사각 + `S`)로 통일했다.
- 인라인 `data:` URI 라 **요청이 0건이고 404 가 날 수 없다.** 색은 DESIGN.md 토큰 `#622599` 로 맞췄다(`admin.html` 이 `#5B2EA6` 로 어긋나 있어 함께 정정 — 변형을 늘리지 않는다).
- `privacy.html` 도 선언이 없어 함께 추가. 회귀에 **'엔트리 9개가 전부 파비콘을 선언한다'** 를 넣어 못 박았다.

**② `rules/failures.md` 신설** (사용자 지시: *"실패기록들을 모두 남기는 파일 하나 만들어서 … 특히 기술부족이나 특정 여력 부족이면 그런 내용들을 모두"*)
- changelog 는 *무엇을 했는가*, failures 는 ***무엇을 틀렸고 왜 그랬는가***. 3절: ① 판단을 틀린 것 ② 검사가 헛돌던 것(기술 부족) ③ **구조적으로 못 하는 것**(여력·권한).
- 이번 세션 것뿐 아니라 저장소 이력에 흩어져 있던 것까지 모았다 — 특히 **F-07(회귀가 실제 시계를 읽어 날짜가 지나면 빨개지는 함정)은 세 번 재발**했다. 번호를 재사용하지 않고 해결돼도 지우지 않는 이유가 그것이다(**재발을 세는 것이 이 문서의 가치**).
- `CLAUDE.md` 작업 기준 ①, `rules/00-index.md`, `handoff.md` 머리말에서 가리킨다. `_middleware` 가 `*.md` 를 막으므로 웹 비공개.

**③ 재현 안 되는 실패 2건은 '원인 특정 실패'로 기록**(F-12) — 그럴듯한 추정을 원인처럼 적지 않았다.
- 대신 `regress-landing`·`regress-service-ended` 에 **4xx 응답의 URL 을 남기는 계측을 상설화**했다. 크롬 콘솔 메시지(`"Failed to load resource: … 404 ()"`)에는 **주소가 안 들어 있어** 다음에도 특정 못 할 뻔했다.

**검증**: 종료 스위트 30→**31** → 14종 **738건**, 2라운드 ALL GREEN.
