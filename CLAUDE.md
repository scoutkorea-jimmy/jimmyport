...
> **⛳️ 최우선 규칙 ③ — 코드 작업·리팩터링 원칙** *(신설 2026-07-15, 사용자 가이드라인 전문 지시)*
...
> - **과잉 설계 금지**(§11): 불필요한 패턴·계층·추상화·미래 대비 구현 금지. 가장 단순하면서 책임이 명확한 구조.
...
> ⚠️ 이 repo 현황: `jamboree-plan/app.js` 는 이미 3,000줄+ 단일 파일에 전역 `state`/`Auth`/렌더러가 얽힌 구조다. **기존 관행을 이유로 복제하지 말 것** — 신규는 분리해서 붙이고, 부채는 §19에 기록한다.

> **⛳️ 최우선 규칙 ② — 작업 대상(프로젝트/앱) 먼저 확인** *(신설 2026-06-27, 사용자 "무조건" 지시)*
> 사용자가 작업·수정·명령을 내리면 **착수 전에 "어떤 대상"인지 먼저 묻고 확정**한다. 두 층위로 구분:
> - **홈페이지/repo 단위**: ① `jimmyport`(이 repo · `scoutingapp.net`) ② `gilwell-media`(별도 repo) ③ `jimmyport` 포트폴리오(예정 · `jimmypark.net` · 새 repo).
> - **앱 단위**(jimmyport repo 내): `/`(랜딩) · `/tour`(+`/tour/admin`) · `/krjam-cardnews` · `/krjam-planning` · `/krjam-dcount`.
> 명령에 대상이 명시되지 않았거나 모호하면 **반드시 질문 후** 진행한다. 단, 현재 대화 흐름에서 대상이 이미 확정된 경우 같은 작업 동안 재질문은 생략한다.

> **⛳️ 최우선 규칙 ④ — UI/UX 디자인 규칙** *(신설 2026-07-17, 사용자 제공 "UI/UX 디자인 핵심 가이드라인 및 2026 트렌드")*
...
> - **정보 완결성**: 시간은 **HH:MM**(오전/오후 구체), 필수 속성 누락 금지. 모바일은 **세로 배열** 기본, 중요 정보 **좌측 정렬** 통일.
...
> ⚠️ **테마 예외(사용자 확정)**: 이 규칙은 "화이트 배경"을 권하지만, `/krjam-planning`은 사용자가 명시적으로 택한 **다크 관제 테마 유지**(v0.9.183~). 흑배경은 유지하되 위의 **계층·최소폰트·카드중첩금지·버튼높이·대비·정보완결성** 원칙은 다크 위에서 적용한다. (2026-07-17 사용자 확인)
