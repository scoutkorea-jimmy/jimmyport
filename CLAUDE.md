...
## 12. 진행 예정 (프런트엔드 UI — 다음 빌드)
> 백엔드·글로벌 모델 완료. 아래 UI가 남음.
- 공개 사이트가 `/api/units`에서 로드(폴백 data.js), 표시 항목 §9 재구성(WOSM·국가·연맹·지구 / 주요활동 / 모집 카테고리 / 연락방법 디폴트 / 인스타).
- **국/영문 i18n — 영어 기본**(외국인 우선), 한국어 보조 토글.
