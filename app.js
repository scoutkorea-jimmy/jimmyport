 ...
 * - 데이터(data.js)와 분리. 관리자 편집본(localStorage)이 있으면 그것으로 미리보기.
 * ========================================================================= */
(function () {
  "use strict";
  ...
  // 관리자 편집본(localStorage)이 있으면 우선 사용, 없으면 data.js 기본값.
  function loadUnits() {
    try {
      var raw = localStorage.getItem(DRAFT_KEY);
      if (raw) {
        var arr = JSON.parse(raw);
        if (Array.isArray(arr) && arr.length) return { units: arr, draft: true };
      }
    } catch (e) { /* noop */ }
    return {
      units: Array.isArray(window.SCOUT_UNITS) ? window.SCOUT_UNITS : [],
      draft: false,
    };
  }
  ...
  function matchUnits(query) {
    ...
    return UNITS.filter(function (u) {
      var hay = [u.name, u.type, u.federation, u.council, u.address]
        .join(" ").toLowerCase();
  ...
  // ── rendering ──────────────────────────────────────────────────────
  function affil(u) {
    return esc(u.federation) + (u.council ? " · " + esc(u.council) : "");
  }
  ...
  function popupHtml(u, anchor) {
    var dist = anchor
      ...
      : "";
    return (
      '<div class="popup-name">' + esc(u.name) +
        (u.type ? ' <span class="popup-type">' + esc(u.type) + "</span>" : "") +
      "</div>" +
      ...
      '<button type="button" class="result-card" data-id="' + esc(u.id) + '">' +
        '<div class="card-top">' +
          ...
          '<div class="card-heading">' +
            '<p class="card-name">' + esc(u.name) +
              (u.type ? ' <span class="card-type">' + esc(u.type) + "</span>" : "") +
            "</p>" +
  ...
  function render(ordered, anchor, statusText) {
    ...
    ordered.forEach(function (u, i) {
      ...
      var marker = L.marker([u.lat, u.lng], {
        icon: unitIcon(rank, false), title: u.name, keyboard: true, alt: u.name,
      });
    ...
    if (anchor) {
      anchorMarker = L.marker([anchor.lat, anchor.lng], {
        icon: anchorIcon(), zIndexOffset: 1000, title: anchor.label || "기준 위치", alt: "기준 위치",
      });
  ...
  function showAll() {
    $input.value = "";
    render(UNITS.slice(), null, "전체 단위대 " + UNITS.length + "곳");
  }

  // ── downloads (홈페이지에서 데이터 받기) ────────────────────────────
  ...
  // ── init ───────────────────────────────────────────────────────────
  function init() {
    if ($badge && loaded.draft) {
      $badge.textContent = "편집본 미리보기";
      $badge.title = "관리자에서 편집한 로컬 데이터로 표시 중입니다 (커밋 전).";
    }
    ...
    if ($dlJson) $dlJson.addEventListener("click", function () {
      triggerDownload("scout-units.json", JSON.stringify(UNITS, null, 2), "application/json");
    });
    if ($dlJs) $dlJs.addEventListener("click", function () {
      triggerDownload("data.js", dataJsText(UNITS), "text/javascript");
    });
