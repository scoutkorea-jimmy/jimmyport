...
function uploadAssets(files, category){
  ...
  arr.forEach(function(f){
    ...
    var p = isImg
      ? downscale(f,1600,0.85).then(function(blob){ return uploadBlob(blob).then(function(r){return r.json();}); }).then(function(d){ return record(d,f,'image/jpeg'); })
      : uploadAttachment(f).then(function(d){ return record(d,f,(d&&d.ct)||f.type||'application/octet-stream'); });
    p.then(function(j){ if(j&&j.ok&&j.asset){ libItems.unshift(j.asset); ok++; } done++; if(done===arr.length){ renderLibrary(); toast('자료 '+ok+'개 추가됨'); } })
     .catch(function(){ done++; if(done===arr.length){ renderLibrary(); toast('일부 업로드 실패'); } });
  });
}
