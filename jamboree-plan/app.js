...
function cubObserverSeeds(){
  ...
  /* 컵 참관단 프로그램 일정표(사용자 제공 이미지, 2026-07-27 개정):
   * 1기 8/5(오후 입소)~8/7(오전 퇴소) · 2기 8/7(오후 입소)~8/9(오전 퇴소). 점심 12–13, 오후 14~. */
  return [
    // ── 1기 ── 8/5 (수) 오후 입소
    it(1,'2026-08-05','14:00','15:00','등록·1기 입소식'),
    it(1,'2026-08-05','15:00','18:00','과정활동'),
    it(1,'2026-08-05','18:00','20:00','저녁식사'),
    it(1,'2026-08-05','20:00','22:00','개영식'),
    it(1,'2026-08-05','22:00','23:00','세면 및 취침'),
    // 1기 8/6 (목)
    it(1,'2026-08-06','06:00','07:00','기상 및 세면'),
    it(1,'2026-08-06','07:00','09:00','아침식사 및 정리'),
    it(1,'2026-08-06','09:00','12:00','과정활동'),
    it(1,'2026-08-06','12:00','13:00','점심식사'),
    it(1,'2026-08-06','14:00','18:00','워터파크'),
    it(1,'2026-08-06','18:00','20:00','저녁식사'),
    it(1,'2026-08-06','20:00','22:00','K-POP 콘서트'),
    it(1,'2026-08-06','22:00','23:00','세면 및 취침'),
    // 1기 8/7 (금) 오전 — 퇴소
    it(1,'2026-08-07','06:00','07:00','기상 및 세면'),
    it(1,'2026-08-07','07:00','09:00','아침식사 및 정리 · 1기 퇴소식'),
    it(1,'2026-08-07','09:00','12:00','DMZ 견학'),
    it(1,'2026-08-07','12:00','13:00','점심식사 및 귀가'),
    // ── 2기 ── 8/7 (금) 오후 입소
    it(2,'2026-08-07','14:00','15:00','등록·2기 입소식'),
    it(2,'2026-08-07','15:00','18:00','과정활동'),
    it(2,'2026-08-07','18:00','20:00','저녁식사'),
    it(2,'2026-08-07','20:00','22:00','컵스나잇'),
    it(2,'2026-08-07','22:00','23:00','세면 및 취침'),
    // 2기 8/8 (토)
    it(2,'2026-08-08','06:00','07:00','기상 및 세면'),
    it(2,'2026-08-08','07:00','09:00','아침식사 및 정리'),
    it(2,'2026-08-08','09:00','12:00','과정활동'),
    it(2,'2026-08-08','12:00','13:00','점심식사'),
    it(2,'2026-08-08','14:00','18:00','워터파크'),
    it(2,'2026-08-08','18:00','20:00','저녁식사'),
    it(2,'2026-08-08','20:00','22:00','폐영식'),
    it(2,'2026-08-08','22:00','23:00','세면 및 취침'),
    // 2기 8/9 (일) 오전 — 퇴소
    it(2,'2026-08-09','06:00','07:00','기상 및 세면'),
    it(2,'2026-08-09','07:00','09:00','아침식사 및 정리 · 2기 퇴소식 · 종교활동'),
    it(2,'2026-08-09','09:00','12:00','DMZ 견학'),
    it(2,'2026-08-09','12:00','13:00','점심식사 및 귀가')
  ];
}
...
/* 컵 참관단 일정 이미지 개정(2026-07-27): 1기 8/4시작 → 8/5, 시간·활동 재구성.
 * 시드는 새 보드에만 주입되므로(mergeCubObservers) 이미 저장된 보드는 자동 갱신되지 않는다.
 * → 구 스케줄 신호(8/4 항목 cub-1-0804-*)가 있을 때만 1회성으로 구 cub 시드-스킴 항목을 걷어내고 신 시드를 넣는다.
 * 자체 가드: 실행 후 8/4 항목이 사라져 다음 로드부터 no-op(멱등). 사용자 생성(mkid) cub 항목·타 트랙은 보존. */
function migrateCubSchedule(){
  var list=ttList();
  if(!list.some(function(t){ return /^cub-1-0804-/.test(t.id||''); })) return 0;   // 구 스케줄 신호 없음 → no-op
  var kept=list.filter(function(t){ return !/^cub-\d-\d{4}-\d{4}$/.test(t.id||''); });   // cub 시드-스킴만 제거(사용자 mkid cub·타 트랙 보존)
  cubObserverSeeds().forEach(function(s){ kept.push(s); });
  state.timetable=kept; saveTimetable();
...
// 보드 열 너비(px) + 열별 숨김(개인 화면 설정, localStorage)
var boardColW=300; try{ var _bcw=parseInt(localStorage.getItem('jamboree-plan:board-colw'),10); if(_bcw>=240&&_bcw<=560) boardColW=_bcw; }catch(e){}
var boardHidden={}; try{ boardHidden=JSON.parse(localStorage.getItem('jamboree-plan:board-hidden')||'{}')||{}; }catch(e){ boardHidden={}; }
function applyBoardColW(){ var b=document.getElementById('board'); if(b) b.style.setProperty('--board-colw', boardColW+'px'); var s=document.getElementById('board-colw'); if(s&&s.value!=String(boardColW)) s.value=boardColW; }
function saveBoardHidden(){ try{ localStorage.setItem('jamboree-plan:board-hidden', JSON.stringify(boardHidden)); }catch(e){} }
function renderBoardCols(){
  var box=document.getElementById('board-cols'); if(!box) return;
  var cols=(Auth.isStaff()?[['inbox','제보 인박스','var(--ch-fb)']]:[]).concat(PIPE);
  box.innerHTML='<span class="bv-clabel">열 표시</span>'+cols.map(function(c){
    var off=!!boardHidden[c[0]];
    return '<button type="button" class="bv-col'+(off?' off':'')+'" data-colv="'+c[0]+'" aria-pressed="'+(!off)+'"><span class="pin" style="background:'+c[2]+'"></span>'+esc(c[1])+'</button>';
  }).join('');
}
...
function renderBoard(){
  ...
  PIPE.forEach(function(def){
    if(boardHidden[def[0]]) return;   // 숨긴 열은 렌더하지 않음(집계·진행률은 그대로)
  ...
  renderBoardCols(); applyBoardColW();
...
function defaultMeals(){
  ...
  return {
    ...
    staff:{   // 운영요원 식사메뉴 확정본(2026-07-28 PDF). 8/3 조·중 없음(입영 14:00)·8/9 중·석 없음(귀가). 음료/디저트는 [ ]. 조식 고정메뉴·제철과일은 MEAL_NOTE.
      '2026-08-03':{ b:'', l:'', d:C('[망고음료]','쌀밥','닭개장','오징어야채핫바','메추리알조림','건파래볶음','김치') },
      '2026-08-04':{ b:C('쌀밥','소고기미역국','떡갈비&양송이','김치'), l:C('[요구르트]','쌀밥','비프카레','맑은김치국','통등심돈까스','시금치나물','블루베리샐러드','김치'), d:C('[포도음료]','쌀밥','건새우아욱국','파채소불고기','야채계란찜','무말랭이무침','김치') },
      '2026-08-05':{ b:C('쌀밥','소고기우거지국','고등어무조림','김치'), l:C('[요플레포도맛]','쌀밥','황태무국','닭볶음탕','김말이강정','오이양파무침','김치'), d:C('[요거트푸딩]','쌀밥','사골떡국','단호박순살갈비찜','갈비만두찜','치커리유자청무침','김치') },
      '2026-08-06':{ b:C('쌀밥','쑥갓어묵탕','미트볼야채볶음','김치'), l:C('화이트패스츄리빵','빅요구르트','초코퍼지휘낭시애','쫄깃한메추리알','미니팝콘','복숭아곤약젤리'), d:C('[오색경단]','장각삼계탕','쌀밥','김치전','오이고추쌈장무침','요구르트','김치') },
      '2026-08-07':{ b:C('쌀밥','소고기무국','메추리알장조림','김치'), l:C('초코칩머핀','아침에주스포도','우리밀바나나빵','뽀로로두부봉','국산콩두부칩','쭈욱짜먹는애플'), d:C('[사과주스]','삼겹보쌈','쌀밥','얼갈이된장국','비빔막국수','배추된장무침','김치') },
      '2026-08-08':{ b:C('돈육짜장덮밥','팽이장국','등심탕수육','김치'), l:C('미니딸기샌드','레몬에이드','크림치즈휘낭시애','구운계란','그레인바이트초코','감귤퐁당컵과일'), d:C('[카프리썬]','쌀밥','된장찌개','제육볶음','한식잡채','부추겉절이','김치') },
      '2026-08-09':{ b:C('쌀밥','얼큰계란감자국','떡갈비야채조림','김치'), l:'', d:'' }
    }
  };
}
...
/* 운영요원(staff) 식사메뉴 확정본 교체(2026-07-28 PDF) — 기존 seed 는 조식이 하루 밀리고(8/3 조식 존재) 오타가 있었다.
 * 저장된 보드는 1회성으로 staff 전체를 확정본으로 교체한다. 가드: 신 데이터 표식(8/8 조식 '돈육짜장덮밥')이 있으면 no-op(멱등).
 * crew_n(일반식)·crew_s(특별식)는 건드리지 않는다. */
function migrateMealsStaff(){
  var m=mealsData(), st=m.staff||{};
  if(/돈육짜장덮밥/.test((st['2026-08-08']||{}).b||'')) return 0;   // 이미 확정본 → no-op
  m.staff=defaultMeals().staff; saveMeals();
  console.info('[마이그레이션] 운영요원(staff) 식사메뉴 확정본 교체');
  return 1;
}
...
function onAuthed(){ document.documentElement.classList.add('pw-ok'); reflectAuthUI(); loadNews(); loadNoti(); loadBoard(); resetAdminIdle(); }
...
function fmtNewsShort(iso){
  try{ var d=new Date(iso); return (d.getMonth()+1)+'/'+d.getDate()+' '+String(d.getHours()).padStart(2,'0')+':'+String(d.getMinutes()).padStart(2,'0'); }
  catch(e){ return fmtNewsTime(iso); }
}
...
/* 목록 정렬·거르기 (v0.9.254)
   기본은 **우선순위 높은 순** — 오늘 뭘 먼저 챙길지가 목록 맨 위에 있어야 한다.
   같은 우선순위면 최신 글이 위로. 머리글을 누르면 그 열 기준으로 바뀐다. */
var newsSort={key:'pri', dir:'desc'};
var newsFilter={stage:'', en:'', person:'', dept:'', q:''};
function newsFilterOn(){ return !!(newsFilter.stage||newsFilter.en||newsFilter.person||newsFilter.dept||newsFilter.q); }
function newsPeopleAll(){
  var seen={}, out=[];
  (newsItems||[]).forEach(function(a){ [a.reporter,a.photographer].forEach(function(n){ n=(n||'').trim(); if(n&&!seen[n]){ seen[n]=1; out.push(n); } }); });
  return out.sort();
}
function newsDeptsAll(){
  var seen={}, out=[];
  (newsItems||[]).forEach(function(a){ (a.depts||[]).forEach(function(d){ d=(d||'').trim(); if(d&&!seen[d]){ seen[d]=1; out.push(d); } }); });
  return out.sort();
}
function newsMatch(a){
  var f=newsFilter;
  if(f.stage && newsStDef(a.stage)[0]!==f.stage) return false;
  if(f.en && (a.en||'')!==f.en) return false;
  if(f.person && (a.reporter||'')!==f.person && (a.photographer||'')!==f.person) return false;
  if(f.dept && (a.depts||[]).indexOf(f.dept)<0) return false;
  if(f.q){
    var q=f.q.toLowerCase();
    var hay=[a.title,a.subtitle,a.authorName,a.reporter,a.photographer,(a.tags||[]).join(' '),(a.depts||[]).join(' ')].join(' ').toLowerCase();
    if(hay.indexOf(q)<0) return false;
  }
  return true;
}
function newsSorted(list, order){
  var k=newsSort.key, dir=newsSort.dir==='asc'?1:-1;
  var val=function(a){
    if(k==='pri') return +(a.priority||0);
    if(k==='no') return order[a.id]||0;
    if(k==='title') return (a.title||'');
    if(k==='author') return (a.authorName||a.author||'');
    if(k==='date') return String(a.createdAt||'');
    if(k==='stage') return ['draft','reviewed','published'].indexOf(newsStDef(a.stage)[0]);
    return 0;
  };
  return list.slice().sort(function(x,y){
    var vx=val(x), vy=val(y), c;
    if(typeof vx==='string') c=vx.localeCompare(vy); else c=(vx-vy);
    if(c) return c*dir;
    return String(y.createdAt||'').localeCompare(String(x.createdAt||''));   // 같으면 최신이 위
  });
}
function newsSortArrow(k){ return newsSort.key===k ? (newsSort.dir==='asc'?' ▲':' ▼') : ''; }
function newsTh(k, label, cls){
  return '<th class="'+(cls||'')+' srt'+(newsSort.key===k?' on':'')+'" data-news-sort="'+k+'" title="눌러서 정렬">'+esc(label)+newsSortArrow(k)+'</th>';
}
function renderNewsFilter(){
  var sel=function(id, cur, opts, ph){
    return '<select class="ti nf" data-nf="'+id+'"><option value="">'+esc(ph)+'</option>'+
      opts.map(function(o){ return '<option value="'+esc(o[0])+'"'+(cur===o[0]?' selected':'')+'>'+esc(o[1])+'</option>'; }).join('')+'</select>';
  };
  return '<div class="newsfilter">'+
    sel('stage', newsFilter.stage, NEWS_ST.map(function(d){ return [d[0],d[1]]; }), '단계 전체')+
    sel('en', newsFilter.en, NEWS_EN.filter(function(d){ return d[0]; }).map(function(d){ return [d[0],d[1]]; }), '영문 전체')+
    sel('person', newsFilter.person, newsPeopleAll().map(function(n){ return [n,n]; }), '담당자 전체')+
    sel('dept', newsFilter.dept, newsDeptsAll().map(function(n){ return [n,n]; }), '협조부서 전체')+
    '<input class="ti nf nfq" data-nf="q" type="search" placeholder="제목 · 태그 · 담당자 찾기" value="'+esc(newsFilter.q)+'">'+
    (newsFilterOn()?'<button type="button" class="btn xs ghost" data-news-nfclear="1">필터 해제</button>':'')+
    '</div>';
}
/* 사진 담당자 (v0.9.251) — 홍보부 인원에서 고르거나 직접 입력한다.
   ⚠️ 인원 id 가 아니라 **이름**을 저장한다. 명단에서 빠져도 "누가 찍기로 했는지"는 남아야 한다. */
function newsPhotoOptions(){
  var seen={}, out=[];
  (rosterList()||[]).forEach(function(r){ var n=(r&&r.name||'').trim(); if(n&&!seen[n]){ seen[n]=1; out.push(n); } });
  return out;
}
/* 협조부서 후보 — 협조 연락처에 적힌 소속 + 자주 쓰는 본부 이름. 직접 입력도 된다. */
var NEWS_DEPT_BASE=['기획조정본부','운영본부','급식편의본부','안전본부','시설본부','프로그램본부','국제본부','홍보부','외부 · 언론'];
function newsDeptOptions(){
  var seen={}, out=[];
  (contactList()||[]).forEach(function(c){ var o=(c&&c.org||'').trim(); if(o&&!seen[o]){ seen[o]=1; out.push(o); } });
  NEWS_DEPT_BASE.forEach(function(o){ if(!seen[o]){ seen[o]=1; out.push(o); } });
  return out;
}
function newsDeptDatalist(id){
  return '<datalist id="'+id+'">'+newsDeptOptions().map(function(n){ return '<option value="'+esc(n)+'"></option>'; }).join('')+'</datalist>';
}
function newsPhotoDatalist(id){
  return '<datalist id="'+id+'">'+newsPhotoOptions().map(function(n){ return '<option value="'+esc(n)+'"></option>'; }).join('')+'</datalist>';
}
/* ── 우선순위 별점 (v0.9.252) ── 0~5, 반개 단위.
   별 하나가 버튼 하나이고, 누른 x 좌표가 별의 왼쪽 절반이면 .5 다(별 10개를 늘어놓으면 손가락으로 못 누른다).
   같은 값을 다시 누르면 0(미지정)으로 지운다. 색만으로 말하지 않도록 숫자도 함께 읽어 준다. */
function newsStars(a, editable){
  var p=+(a.priority||0), out='';
  for(var i=1;i<=5;i++){
    var fill = p>=i ? 'full' : (p>=i-0.5 ? 'half' : 'none');
    out += '<button type="button" class="star '+fill+'"'+(editable?(' data-news-star="'+esc(a.id)+'~'+i+'"'):' disabled')+
      ' aria-label="'+i+'점"><span class="s-bg">★</span><span class="s-fg">★</span></button>';
  }
  return '<div class="stars'+(p?'':' zero')+'" role="group" aria-label="우선순위 '+(p||0)+'점 (5점 만점)" title="우선순위 '+(p||0)+' / 5">'+
    out+'<span class="starnum">'+(p?p.toFixed(1).replace(/\.0$/,''):'—')+'</span></div>';
}
function setNewsPriority(id, val){
  var a=newsItems.filter(function(x){return x.id===id;})[0]; if(!a) return;
  if(!canFlagNews(a)){ toast('권한이 없습니다'); return; }
  var v=Math.min(5, Math.max(0, Math.round(val*2)/2));
  if(+(a.priority||0)===v) v=0;                       // 같은 값을 다시 누르면 지운다
  var prev=a.priority||0; a.priority=v; renderNews(); if(newsView===id) renderNewsView();
  fetch('/api/jp-news',{method:'POST',headers:authJsonHeaders(),body:JSON.stringify({action:'flags',id:id,priority:v})})
    .then(function(r){ if(r.status===401){ authExpired(); return null; } if(r.status===403){ toast('권한이 없습니다'); return null; } return r.json(); })
    .then(function(j){ if(!j||!j.ok){ a.priority=prev; renderNews(); if(newsView===id) renderNewsView(); if(j) toast('저장 실패'); } })
    .catch(function(){ a.priority=prev; renderNews(); toast('네트워크 오류'); });
}
/* ── 영문 가공 검토 (v0.9.252) ──
   "이 기사는 영문으로도 내보내야 하나?"를 기사마다 남긴다. 카드뉴스 가공과 같은 축이라 버전은 안 오른다.
   목록의 칩을 누르면 미검토 → 필요 → 완료 → 불필요 순으로 돈다(모달에는 버튼 4개로 직접 고른다). */
var NEWS_EN=[['','미검토','var(--surface-3)','var(--muted)'],
             ['need','영문 필요','var(--st-draft-bg)','var(--st-draft)'],
             ['done','영문 완료','var(--st-ready-bg)','var(--st-ready)'],
             ['skip','불필요','var(--surface-3)','var(--faint,#9AA39C)']];
function newsEnDef(v){ for(var i=0;i<NEWS_EN.length;i++) if(NEWS_EN[i][0]===String(v||'')) return NEWS_EN[i]; return NEWS_EN[0]; }
function newsEnChip(a, editable){
  var d=newsEnDef(a.en);
  return '<button type="button" class="pst enchip" style="background:'+d[2]+';color:'+d[3]+'"'+
    (editable?(' data-news-en="'+esc(a.id)+'" title="눌러서 영문 가공 검토 상태 변경"'):' disabled')+'>'+esc(d[1])+'</button>';
}
function setNewsEn(id, v){
  var a=newsItems.filter(function(x){return x.id===id;})[0]; if(!a) return;
  if(!canFlagNews(a)){ toast('권한이 없습니다'); return; }
  var prev=a.en||'';
  if(v===undefined){ var i=NEWS_EN.map(function(d){return d[0];}).indexOf(prev); v=NEWS_EN[(i+1)%NEWS_EN.length][0]; }
  a.en=v; renderNews(); if(newsView===id) renderNewsView();
  fetch('/api/jp-news',{method:'POST',headers:authJsonHeaders(),body:JSON.stringify({action:'flags',id:id,en:v})})
    .then(function(r){ if(r.status===401){ authExpired(); return null; } if(r.status===403){ toast('권한이 없습니다'); return null; } return r.json(); })
    .then(function(j){ if(!j||!j.ok){ a.en=prev; renderNews(); if(newsView===id) renderNewsView(); if(j) toast('저장 실패'); } })
    .catch(function(){ a.en=prev; renderNews(); toast('네트워크 오류'); });
}
...
function renderArticleComments(a){
  var list=(a.comments||[]).map(function(c){
    ...
    // 어느 판을 보고 쓴 의견인지 — 버전이 올라도 코멘트는 남으므로 기준 버전이 없으면 맥락을 잃는다
    var vb=c.v?('<span class="ac-v" title="V'+c.v+' 기준 검수 의견">V'+c.v+'</span>'):'';
    return '<div class="ac-item"><div class="ac-h"><b>'+esc(c.author||'')+'</b>'+vb+'<span>'+esc(fmtNewsTime(c.ts))+'</span>'+del+'</div><div class="ac-t">'+esc(c.text||'').replace(/\n/g,'<br>')+'</div></div>';
  }).join('');
  return '<div class="ac-box">'+(list||'<div class="ac-empty">아직 검수 코멘트가 없습니다.</div>')+
    (Auth.authed()?('<div class="ac-add">'+
      '<textarea class="ac-input" data-ac-input="'+esc(a.id)+'" rows="2" maxlength="4000" placeholder="검수 의견·수정 요청… (줄바꿈은 Enter, 등록은 아래 버튼 또는 &#8984;/Ctrl+Enter)"></textarea>'+
      '<button class="btn xs solid" data-ac-send="'+esc(a.id)+'">등록</button></div>'):'')+'</div>';
}
/* 코멘트 칸 자동 높이 — 줄이 늘면 따라 커지고, 너무 커지면 스크롤로 넘긴다.
   textarea 로 바꾼 이유: 검수 의견은 한 줄로 끝나지 않는다(사용자 요청). Enter 는 줄바꿈이고,
   등록은 버튼 또는 ⌘/Ctrl+Enter — Enter 로 보내면 두 번째 줄을 못 쓴다. */
function growComment(el){ if(!el) return; el.style.height='auto'; el.style.height=Math.min(el.scrollHeight+2, 260)+'px'; }
function addNewsComment(id){
  ...
  fetch('/api/jp-news',{method:'POST',headers:authJsonHeaders(),body:JSON.stringify({action:'comment',id:id,text:text,authorName:Auth.name||Auth.username})})
    ...
    .then(function(j){ if(j&&j.ok&&j.article){ var i=newsItems.map(function(x){return x.id;}).indexOf(id); if(i>=0) newsItems[i]=j.article; renderNews(); renderNoti(); if(newsView===id) renderNewsView(); } else toast('등록 실패'); })
    .catch(function(){ toast('네트워크 오류'); });
}
...
function renderNews(){
  ...
  if(bar){
    if(sel.length){
      ...
      var enNeed=newsItems.filter(function(a){ return (a.en||'')==='need'; }).length;
      var vis=newsItems.filter(newsMatch).length;
      bar.innerHTML='<span class="press-stat">전체 <b>'+newsItems.length+'</b></span>'+
        (newsFilterOn()?('<span class="press-stat en">보이는 <b>'+vis+'</b></span>'):'')+
        NEWS_ST.map(function(d){ return '<span class="press-stat">'+esc(d[1])+' <b>'+cnt[d[0]]+'</b></span>'; }).join('')+
        (enNeed?('<span class="press-stat en">영문 필요 <b>'+enNeed+'</b></span>'):'');
    }
  }
  ...
  var shown=newsSorted(newsItems.filter(newsMatch), order);
  var allOn = shown.length>0 && shown.every(function(a){ return !!newsSel[a.id]; });
  var rows=shown.map(function(a){
    ...
    return '<tr class="news-row'+(on?' picked':'')+'">'+
      ...
      '<td class="nr-author">'+esc(a.authorName||a.author||'홍보부원')+
        (a.reporter?('<span class="nr-photog">'+icon('user',11)+' 취재 '+esc(a.reporter)+'</span>'):'')+
        (a.photographer?('<span class="nr-photog">'+icon('image',11)+' 사진 '+esc(a.photographer)+'</span>'):'')+'</td>'+
      '<td class="nr-dept">'+((a.depts||[]).length
        ? (a.depts||[]).slice(0,2).map(function(d){ return '<span class="news-tag dept sm">'+esc(d)+'</span>'; }).join('')
          + ((a.depts||[]).length>2?('<span class="muted">+'+((a.depts||[]).length-2)+'</span>'):'')
        : '<span class="muted">—</span>')+'</td>'+
      '<td class="nr-date">'+esc(fmtNewsShort(a.createdAt))+'</td>'+
      '<td class="nr-pri">'+newsStars(a, canFlagNews(a))+'</td>'+
      '<td class="nr-pub">'+newsStChip(newsStDef(a.stage)[0])+'</td>'+
      '<td class="nr-en">'+newsEnChip(a, canFlag)+'</td>'+
      '<td class="nr-cn">'+cnBtn+'</td>'+
    '</tr>';
  }).join('');
  var head='<tr>'+
    '<th class="nr-ck"><input type="checkbox" class="pck" id="news-ckall"'+(allOn?' checked':'')+' aria-label="전체 선택"></th>'+
    newsTh('no','번호','nr-no')+newsTh('title','제목','nr-title')+newsTh('author','작성자 · 담당','nr-author')+
    '<th class="nr-dept">협조부서</th>'+newsTh('date','작성일','nr-date')+newsTh('pri','우선순위','nr-pri')+
    newsTh('stage','단계','nr-pub')+'<th class="nr-en">영문</th><th class="nr-cn">카드뉴스</th></tr>';
  var none = shown.length===0 ? '<div class="news-empty">조건에 맞는 기사가 없습니다. <button type="button" class="btn xs ghost" data-news-nfclear="1">필터 해제</button></div>' : '';
  box.innerHTML=renderNewsFilter()+none+
    '<div class="tblscroll"><table class="newstbl presstbl"><thead>'+head+'</thead><tbody>'+rows+'</tbody></table></div>';
}
/* ── 기사 읽기 모달 ── 목록에 본문을 끼워 넣지 않는다(보도자료와 같은 방식) */
function openNewsView(id){
  var a=newsItems.filter(function(x){return x.id===id;})[0]; if(!a) return;
  newsView=id; newsVerShow=null; document.getElementById('newsview-scrim').classList.add('show'); renderNewsView();
}
...
function renderNewsView(){
  ...
  var meta=[newsStChip(newsStDef(a.stage)[0]),
    '<span class="nv-ver" title="수정할 때마다 1씩 오릅니다">V'+ver+'</span>',
  ...
  meta.push(newsStars(a, canFlagNews(a)));
  if(a.reporter) meta.push('<span>'+icon('user',13)+' 취재 '+esc(a.reporter)+'</span>');
  if(a.photographer) meta.push('<span>'+icon('image',13)+' 사진 '+esc(a.photographer)+'</span>');
  ...
  body.innerHTML='<div class="pv-meta">'+meta.join('')+'</div>'+
    ...
    ((a.depts||[]).length?('<div class="nv-depts"><span class="nv-lb">협조부서</span>'+
      (a.depts||[]).map(function(d){ return '<span class="news-tag dept">'+esc(d)+'</span>'; }).join('')+'</div>'):'')+
    (canFlagNews(a)?('<div class="nv-photog"><label for="nv-reporter-in">취재 담당자</label>'+
      '<input class="ti" id="nv-reporter-in" type="text" list="nv-photog-list" maxlength="40" placeholder="이름 입력 또는 명단에서 선택" value="'+esc(a.reporter||'')+'">'+
      '<button type="button" class="btn xs solid" data-nv-reporter="'+esc(a.id)+'">배정</button></div>'):'')+
    (canFlagNews(a)?('<div class="nv-photog"><label for="nv-photog-in">사진 담당자</label>'+
      '<input class="ti" id="nv-photog-in" type="text" list="nv-photog-list" maxlength="40" placeholder="이름 입력 또는 명단에서 선택" value="'+esc(a.photographer||'')+'">'+
      newsPhotoDatalist('nv-photog-list')+
      '<button type="button" class="btn xs solid" data-nv-photog="'+esc(a.id)+'">배정</button></div>'):'')+
    '<div class="nvv" id="nv-verview" hidden></div>'+
    renderNewsHistory(a)+
  ...
  if(canFlagNews(a)) t+='<div class="pv-stset en"><span class="pv-lb">영문</span>'+NEWS_EN.map(function(d){
...
/* ── 버전 기록 (v0.9.251) ──
   history 에는 각 판의 내용이 통째로 들어 있다. 목록에서 하나를 고르면 그 판을 그 자리에서 읽는다.
   현재 판은 언제나 목록 맨 위에 있고, 되돌리기(현재 판 교체)는 하지 않는다 — 읽기 전용이다. */
var newsVerOpen={}, newsVerShow=null;
function renderNewsHistory(a){
  var h=(a.history||[]).slice().sort(function(x,y){ return (y.v||0)-(x.v||0); });
  if(h.length<2 && !a.historyTrimmed) return '';   // 판이 하나뿐이면 기록을 보여줄 게 없다
  var open=!!newsVerOpen[a.id];
  var rows=h.map(function(sn){
    var cur=(sn.v===(a.version||1));
    return '<button type="button" class="nvh-row'+(newsVerShow===sn.v?' on':'')+'" data-nv-ver="'+sn.v+'">'+
      '<span class="nvh-v">V'+sn.v+'</span>'+
      '<span class="nvh-t">'+esc(sn.title||'(제목 없음)')+'</span>'+
      '<span class="nvh-m">'+esc(sn.byName||sn.by||'')+' · '+esc(fmtNewsTime(sn.at))+(cur?' · 현재':'')+'</span></button>';
  }).join('');
  var trimmed=a.historyTrimmed?('<div class="nvh-note">오래된 '+a.historyTrimmed+'판은 저장 용량 때문에 보관하지 않습니다.</div>'):'';
  return '<div class="nvh">'+
    '<button type="button" class="nvh-h" data-nv-hist="'+esc(a.id)+'" aria-expanded="'+open+'">'+
      '<span>'+(open?'▾':'▸')+' 버전 기록 '+h.length+'판</span></button>'+
    (open?('<div class="nvh-list">'+rows+trimmed+'</div>'):'')+'</div>';
}
function renderNewsVerView(){
  var a=newsItems.filter(function(x){return x.id===newsView;})[0]; if(!a) return;
  var wrap=document.getElementById('nv-verview'); if(!wrap) return;
  var sn=(a.history||[]).filter(function(x){ return x.v===newsVerShow; })[0];
  if(!sn || newsVerShow===(a.version||1)){ wrap.innerHTML=''; wrap.hidden=true; return; }
  var imgs=(sn.images||[]).map(function(u){ return '<img class="nv-img" src="'+esc(u)+'" alt="" data-lb="'+esc(u)+'" loading="lazy">'; }).join('');
  var tags=(sn.tags||[]).map(function(t){ return '<span class="news-tag">#'+esc(t)+'</span>'; }).join('');
  wrap.hidden=false;
  wrap.innerHTML='<div class="nvv-h"><b>V'+sn.v+'</b> <span>'+esc(sn.byName||sn.by||'')+' · '+esc(fmtNewsTime(sn.at))+'</span>'+
...
function setNewsReporter(id){
  var a=newsItems.filter(function(x){return x.id===id;})[0]; if(!a) return;
  if(!canFlagNews(a)){ toast('권한이 없습니다'); return; }
  var el=document.getElementById('nv-reporter-in'); if(!el) return;
  var name=(el.value||'').trim().slice(0,40);
  fetch('/api/jp-news',{method:'POST',headers:authJsonHeaders(),body:JSON.stringify({action:'flags',id:id,reporter:name})})
    .then(function(r){ if(r.status===401){ authExpired(); return null; } if(r.status===403){ toast('권한이 없습니다'); return null; } return r.json(); })
    .then(function(j){ if(j&&j.ok){ a.reporter=name; renderNews(); renderNewsView();
        toast(name?('취재 담당자를 '+name+'(으)로 배정했습니다'):'취재 담당자를 비웠습니다'); } else if(j) toast('배정 실패'); })
    .catch(function(){ toast('네트워크 오류'); });
}
function setNewsPhotographer(id){
  var a=newsItems.filter(function(x){return x.id===id;})[0]; if(!a) return;
  if(!canFlagNews(a)){ toast('권한이 없습니다'); return; }
  var el=document.getElementById('nv-photog-in'); if(!el) return;
  var name=(el.value||'').trim().slice(0,40);
  fetch('/api/jp-news',{method:'POST',headers:authJsonHeaders(),body:JSON.stringify({action:'flags',id:id,photographer:name})})
    .then(function(r){ if(r.status===401){ authExpired(); return null; } if(r.status===403){ toast('권한이 없습니다'); return null; } return r.json(); })
    .then(function(j){ if(j&&j.ok){ a.photographer=name; renderNews(); renderNewsView();
        toast(name?('사진 담당자를 '+name+'(으)로 배정했습니다'):'사진 담당자를 비웠습니다'); } else if(j) toast('배정 실패'); })
    .catch(function(){ toast('네트워크 오류'); });
}
...
/* ── 알림 (v0.9.251) ── 내 기사에 달린 검수 코멘트를 알려 준다.
   ⚠️ 서버에 알림 저장소를 새로 두지 않았다 — 코멘트는 이미 기사 레코드에 있으므로 화면에서 뽑아낸다.
      (KV 스키마를 늘리면 저장 실패·유실 경로가 하나 더 생긴다. 읽음 표시만 기기별 localStorage.)
   대상: 내가 쓴 기사에 달린 남의 코멘트 + 내가 코멘트를 단 기사에 달린 남의 코멘트(대화 참여자). */
var NOTI_LS='jamboree-plan:noti-read', notiOpen=false;
/* 서버 알림함(v0.9.252). 읽음 시각이 서버에 있어 기기 간에 맞는다.
   ⚠️ 서버가 안 되면 화면에서 뽑는 방식(v0.9.251)으로 되돌아간다 — 알림 하나 때문에 보드가 멈추면 안 된다. */
var notiBox=null;   // {items, readAt} · null 이면 아직 못 받았다(로컬 유도 사용)
function loadNoti(){
  if(!Auth.authed()) return Promise.resolve(null);
  return fetch('/api/jp-noti',{headers:authHeader()})
    .then(function(r){ return r.ok?r.json():null; })
    .then(function(j){ if(j&&j.ok){ notiBox={items:j.items||[], readAt:j.readAt||''}; renderNoti(); } return j; })
    .catch(function(){ return null; });
}
function notiReadAt(){
  if(notiBox) return notiBox.readAt||'';
  try{ var m=JSON.parse(localStorage.getItem(NOTI_LS)||'{}'); return m[Auth.username||'?']||''; }catch(e){ return ''; }
}
function notiMarkAll(){
  var now=new Date().toISOString();
  if(notiBox) notiBox.readAt=now;
  try{ var m=JSON.parse(localStorage.getItem(NOTI_LS)||'{}'); m[Auth.username||'?']=now;
    localStorage.setItem(NOTI_LS, JSON.stringify(m)); }catch(e){}   // 서버가 안 되도 이 기기에서는 읽음으로
  renderNoti();
  if(Auth.authed()) fetch('/api/jp-noti',{method:'POST',headers:authJsonHeaders(),body:JSON.stringify({action:'read'})})
    .then(function(r){ return r.ok?r.json():null; })
    .then(function(j){ if(j&&j.ok){ notiBox={items:j.items||[], readAt:j.readAt||''}; renderNoti(); } })
    .catch(function(){});
}
function notiItems(){
  // 서버 알림함이 있으면 그것이 사실이다(다른 기기에서 읽은 것까지 반영된다)
  if(notiBox) return notiBox.items.map(function(n){
    return { id:n.articleId, title:n.title, who:n.whoName||n.who, text:n.text, ts:n.ts, v:n.v||0,
             mine:(newsItems||[]).some(function(a){ return a.id===n.articleId && a.author===Auth.username; }) };
  }).slice(0,50);
  return notiItemsLocal();
}
function notiItemsLocal(){
  var me=Auth.username||'', out=[];
  if(!me) return out;
  (newsItems||[]).forEach(function(a){
    var cs=a.comments||[]; if(!cs.length) return;
    var mine = (a.author===me) || cs.some(function(c){ return c.username===me; });
    if(!mine) return;
    cs.forEach(function(c){ if(c.username===me) return;
      out.push({ id:a.id, title:a.title||'(제목 없음)', who:c.author||'', text:c.text||'', ts:c.ts||'', v:c.v||0, mine:a.author===me }); });
  });
  out.sort(function(x,y){ return String(y.ts).localeCompare(String(x.ts)); });
  return out.slice(0,50);
}
function notiUnread(){ var at=notiReadAt(); return notiItems().filter(function(n){ return String(n.ts)>at; }); }
function renderNoti(){
  var btn=document.getElementById('noti-btn'), cnt=document.getElementById('noti-count'), list=document.getElementById('noti-list');
  if(!btn) return;
  btn.style.display = Auth.authed()?'':'none';
  var un=notiUnread(), at=notiReadAt();
  if(cnt){ cnt.hidden = un.length===0; cnt.textContent = un.length>99?'99+':String(un.length); }
  btn.classList.toggle('has', un.length>0);
  if(!list) return;
  var items=notiItems();
  list.innerHTML = items.length ? items.map(function(n){
    var isNew=String(n.ts)>at;
    return '<button type="button" class="notiitem'+(isNew?' new':'')+'" data-noti="'+esc(n.id)+'">'+
      '<span class="ni-h"><b>'+esc(n.who)+'</b>'+(n.v?('<span class="ac-v">V'+n.v+'</span>'):'')+
        '<span class="ni-t">'+esc(fmtNewsTime(n.ts))+'</span></span>'+
      '<span class="ni-a">'+esc(n.mine?'내 기사':'참여한 기사')+' · '+esc(n.title)+'</span>'+
      '<span class="ni-x">'+esc(String(n.text).replace(/\s+/g,' ').slice(0,60))+'</span></button>';
  }).join('') : '<div class="noti-empty">새 검수 코멘트가 없습니다.</div>';
}
function toggleNoti(on){
  var p=document.getElementById('noti-panel'), b=document.getElementById('noti-btn');
  notiOpen = (on===undefined) ? !notiOpen : !!on;
  if(p) p.hidden=!notiOpen;
  if(b) b.setAttribute('aria-expanded', notiOpen?'true':'false');
  if(notiOpen) renderNoti();
}
/* 새 코멘트가 도착하면 한 번만 알린다 — 목록을 다시 불러올 때마다 같은 걸 또 띄우지 않게 마지막 개수를 기억한다 */
var notiSeenTs='';
function notiPing(){
  loadNoti();
  var un=notiUnread(); renderNoti();
  if(!un.length) return;
  var top=un[0].ts;
  if(top && top!==notiSeenTs){ notiSeenTs=top;
    if(un.length===1) toast(un[0].who+'님이 「'+un[0].title+'」에 검수 코멘트를 남겼습니다');
    else toast('새 검수 코멘트 '+un.length+'건');
  }
}
function openNewsEditor(id){
  ...
  newsEdit = src
    ? {id:src.id, title:src.title||'', subtitle:src.subtitle||'', body:src.body||'', images:(src.images||[]).slice(0,3), tags:(src.tags||[]).slice(0,10), stage:newsStDef(src.stage)[0], photographer:src.photographer||'', reporter:src.reporter||'', depts:(src.depts||[]).slice(0,10)}
    : {title:'', subtitle:'', body:'', images:[], tags:[], stage:'draft', photographer:'', reporter:'', depts:[]};
...
function renderNewsEditor(){
  ...
  b.innerHTML=
    ...
    '<div class="news-photog"><label for="news-reporter">취재 담당자</label>'+
      '<input class="ti" id="news-reporter" type="text" list="news-photog-list" maxlength="40" placeholder="이름 입력 또는 명단에서 선택" value="'+esc(newsEdit.reporter||'')+'">'+
      '<label for="news-photog">사진 담당자</label>'+
      '<input class="ti" id="news-photog" type="text" list="news-photog-list" maxlength="40" placeholder="이름 입력 또는 명단에서 선택" value="'+esc(newsEdit.photographer||'')+'">'+
      newsPhotoDatalist('news-photog-list')+'</div>'+
    '<div class="news-depts"><label>협조부서 <span class="muted">(최대 10개 · Enter 로 추가)</span></label><div id="news-deptsec"></div></div>'+
  ...
  var pg=document.getElementById('news-photog'); if(pg) pg.oninput=function(){ newsEdit.photographer=this.value; };
  var rp=document.getElementById('news-reporter'); if(rp) rp.oninput=function(){ newsEdit.reporter=this.value; };
  renderNewsDepts();
...
/* 협조부서 칩 — 태그와 같은 방식이되 목록에서 고를 수 있게 datalist 를 붙인다. */
function renderNewsDepts(){
  var sec=document.getElementById('news-deptsec'); if(!sec||!newsEdit) return;
  var chips=(newsEdit.depts||[]).map(function(t,i){
    return '<span class="news-tag dept">'+esc(t)+'<button type="button" class="news-tagx" data-news-dept-del="'+i+'" aria-label="협조부서 빼기">'+icon('x',11)+'</button></span>'; }).join('');
  var input=(newsEdit.depts||[]).length<10 ? '<input class="ti news-taginput" id="news-deptinput" type="text" list="news-dept-list" maxlength="40" placeholder="부서 입력 후 Enter">' : '';
  sec.innerHTML='<div class="news-tags">'+chips+'</div>'+input+newsDeptDatalist('news-dept-list');
  var ip=document.getElementById('news-deptinput');
  if(ip){
    ip.onkeydown=function(e){ if(e.key==='Enter'||e.key===','){ e.preventDefault(); addNewsDept(this.value); this.value=''; } };
    ip.onblur=function(){ if(this.value.trim()){ addNewsDept(this.value); this.value=''; } };
  }
  sec.onclick=function(e){ var x=e.target.closest('[data-news-dept-del]'); if(!x) return;
    newsEdit.depts.splice(+x.getAttribute('data-news-dept-del'),1); renderNewsDepts(); };
}
function addNewsDept(v){
  if(!newsEdit) return;
  if(!newsEdit.depts) newsEdit.depts=[];
  String(v||'').split(',').forEach(function(t){
    t=t.trim().replace(/\s+/g,' ').slice(0,40);
    if(t && newsEdit.depts.indexOf(t)<0 && newsEdit.depts.length<10) newsEdit.depts.push(t);
  });
  renderNewsDepts();
  var ip=document.getElementById('news-deptinput'); if(ip) ip.focus();
}
...
function renderNewsPhotos(){
  var sec=document.getElementById('news-photos-sec'); if(!sec||!newsEdit) return;
...
function handleNewsFiles(files){
  ...
  var over=all.filter(function(f){ return f.size>10*1024*1024; }).length;
  var list=all.filter(function(f){ return f.size<=10*1024*1024; }).slice(0,remaining);
  if(over) toast(over+'장은 10MB를 넘어 제외했습니다');
  if(!list.length) return;
  toast('사진 업로드 중…');
  (function next(){
    if(!list.length){ renderNewsPhotos(); return; }
    var f=list.shift();
    downscale(f,1600,0.85).then(function(blob){ return uploadBlob(blob); })
...
function commitNews(){
  ...
  var payload={title:title, subtitle:(newsEdit.subtitle||'').trim(), body:body, images:newsEdit.images,
               tags:newsEdit.tags||[], stage:newsEdit.stage||'draft', photographer:(newsEdit.photographer||'').trim(),
               reporter:(newsEdit.reporter||'').trim(), depts:newsEdit.depts||[],
...
var pressItems=[], pressLoaded=false, pressEdit=null, pressBodyEditor=null;
var pressSel={};        // 다중 선택 — id -> true
var pressView=null;     // 읽기 모달에 띄운 기사 id
/* 기사 단계 3종 (v0.9.249). 값은 서버 화이트리스트(functions/api/jp-press.js)와 반드시 같아야 한다.
   구 2단계('released')는 서버가 읽을 때 'published' 로 승격해 준다. */
var PRESS_ST=[['draft','초안','var(--st-planned-bg)','var(--st-planned)'],
              ['reviewed','최종검수 완료','var(--st-draft-bg)','var(--st-draft)'],
              ['published','퍼블리싱 완료','var(--st-ready-bg)','var(--st-ready)']];
function pressStDef(v){ v=(v==='released')?'published':v; for(var i=0;i<PRESS_ST.length;i++) if(PRESS_ST[i][0]===v) return PRESS_ST[i]; return PRESS_ST[0]; }
function pressStChip(v){ var d=pressStDef(v); return '<span class="pst" style="background:'+d[2]+';color:'+d[3]+'">'+esc(d[1])+'</span>'; }
function pressSelIds(){ return Object.keys(pressSel).filter(function(k){ return pressSel[k] && pressItems.some(function(x){return x.id===k;}); }); }
function loadPress(){
...
/* 예전에는 목록에서 행을 펼쳐 본문을 보여줬다(아코디언). 게시판처럼 모달로 연다. */
function openPressView(id){
  var a=pressItems.filter(function(x){return x.id===id;})[0]; if(!a) return;
  pressView=id;
  document.getElementById('pressview-scrim').classList.add('show');
  renderPressView();
}
function closePressView(){ pressView=null; var sc=document.getElementById('pressview-scrim'); if(sc) sc.classList.remove('show'); }
function renderPressView(){
  var a=pressItems.filter(function(x){return x.id===pressView;})[0];
  var body=document.getElementById('pv-body'), head=document.getElementById('pv-title'), foot=document.getElementById('pv-tools');
  if(!a||!body){ closePressView(); return; }
  head.textContent=a.title||'(제목 없음)';
  var meta=[];
  meta.push(pressStChip(a.status));
  if(a.date) meta.push('<span>'+icon('calendar',13)+' 배포일 '+esc(a.date)+'</span>');
  if(a.contact) meta.push('<span>'+icon('user',13)+' 담당 '+esc(a.contact)+'</span>');
  if(a.outlets) meta.push('<span>'+icon('megaphone',13)+' 매체 '+esc(a.outlets)+'</span>');
  meta.push('<span>작성 '+esc(a.authorName||'')+' · '+esc(fmtNewsTime(a.createdAt))+((a.updatedAt&&a.updatedAt!==a.createdAt)?' · 수정됨':'')+'</span>');
  body.innerHTML='<div class="pv-meta">'+meta.join('')+'</div>'+
    (a.body?('<div class="news-text pv-text">'+pressBodyHtml(a.body)+'</div>'):'<div class="news-text muted">본문 없음</div>')+
    pressAttHtml(a);
  var t='';
  if(canEditPress(a)){
    t+='<div class="pv-stset">'+PRESS_ST.map(function(d){
      return '<button type="button" class="btn sm'+(pressStDef(a.status)[0]===d[0]?' solid':' ghost')+'" data-pv-st="'+d[0]+'">'+esc(d[1])+'</button>'; }).join('')+'</div>';
  }
  foot.innerHTML=t;
  var ed=document.getElementById('pv-edit'), dl=document.getElementById('pv-del');
  if(ed) ed.style.display=canEditPress(a)?'':'none';
  if(dl) dl.style.display=canEditPress(a)?'':'none';
}
/* 3단계라 '토글'이 성립하지 않는다 — 원하는 단계를 직접 지정한다. */
function setPressStatus(id, st){
  var a=pressItems.filter(function(x){return x.id===id;})[0]; if(!a) return Promise.resolve(false);
  if(!canEditPress(a)){ toast('권한이 없습니다'); return Promise.resolve(false); }
  return fetch('/api/jp-press',{method:'POST',headers:authJsonHeaders(),body:JSON.stringify({action:'status',id:id,status:st})})
    .then(function(r){ if(r.status===401){ authExpired(); return null; } return r.json(); })
    .then(function(j){ if(j&&j.ok&&j.item){ var i=pressItems.map(function(x){return x.id;}).indexOf(id); if(i>=0) pressItems[i]=j.item; return true; } return false; })
    .catch(function(){ toast('네트워크 오류'); return false; });
}
...
function renderPress(){
  ...
  // 선택 상태에서 사라진 글은 정리한다(삭제 직후 유령 선택 방지)
  Object.keys(pressSel).forEach(function(k){ if(!pressItems.some(function(x){return x.id===k;})) delete pressSel[k]; });
  var sel=pressSelIds();
  if(bar){
    if(sel.length){
      // 선택이 있으면 통계 대신 일괄 작업 바 — 지금 할 수 있는 일만 보여준다
      bar.innerHTML='<div class="press-bulk"><b>'+sel.length+'개 선택</b>'+
        '<span class="pb-sep"></span><span class="pb-lb">단계 일괄 변경</span>'+
        PRESS_ST.map(function(d){ return '<button class="btn xs ghost" data-press-bulk-st="'+d[0]+'">'+esc(d[1])+'</button>'; }).join('')+
        '<span class="spacer"></span>'+
        '<button class="btn xs ghost danger" data-press-bulk-del="1">'+icon('trash',12)+' 선택 삭제</button>'+
        '<button class="btn xs ghost" data-press-selnone="1">선택 해제</button></div>';
    } else {
      var cnt={}; PRESS_ST.forEach(function(d){ cnt[d[0]]=0; });
      pressItems.forEach(function(a){ cnt[pressStDef(a.status)[0]]++; });
      bar.innerHTML='<span class="press-stat">전체 <b>'+pressItems.length+'</b></span>'+
        PRESS_ST.map(function(d){ return '<span class="press-stat">'+esc(d[1])+' <b>'+cnt[d[0]]+'</b></span>'; }).join('');
    }
  }
  ...
  var allOn = pressItems.length>0 && sel.length===pressItems.length;
  var rows=pressItems.map(function(a){
    var atn=(a.attachments||[]).length, on=!!pressSel[a.id];
    return '<tr class="news-row'+(on?' picked':'')+'">'+
      '<td class="nr-ck"><input type="checkbox" class="pck" data-press-ck="'+esc(a.id)+'"'+(on?' checked':'')+' aria-label="선택"></td>'+
      '<td class="nr-no">'+(order[a.id]||'')+'</td>'+
      '<td class="nr-title"><button type="button" class="nr-titlebtn" data-press-view="'+esc(a.id)+'"><span class="nr-tt">'+esc(a.title||'(제목 없음)')+'</span>'+(atn?('<span class="nr-badge">'+icon('paperclip',11)+atn+'</span>'):'')+'</button></td>'+
      '<td class="nr-date">'+esc(a.date||'—')+'</td>'+
      '<td class="nr-author">'+esc(a.contact||a.authorName||'—')+'</td>'+
      '<td class="nr-pub">'+pressStChip(a.status)+'</td>'+
    '</tr>';
  }).join('');
...
function pressBulkStatus(st){
  var ids=pressSelIds(), mine=ids.filter(function(id){ return canEditPress(pressItems.filter(function(x){return x.id===id;})[0]); });
  if(!mine.length){ toast('변경할 수 있는 글이 없습니다'); return; }
  var skipped=ids.length-mine.length;
  Promise.all(mine.map(function(id){ return setPressStatus(id, st); })).then(function(){
    renderPress(); if(pressView) renderPressView();
    toast(mine.length+'건을 '+pressStDef(st)[1]+'(으)로 바꿨습니다'+(skipped?(' · 권한 없는 '+skipped+'건 제외'):''));
  });
}
function pressBulkDelete(){
  var ids=pressSelIds(), mine=ids.filter(function(id){ return canEditPress(pressItems.filter(function(x){return x.id===id;})[0]); });
  if(!mine.length){ toast('삭제할 수 있는 글이 없습니다'); return; }
  var skipped=ids.length-mine.length;
  if(!confirm(mine.length+'건을 삭제할까요? (되돌릴 수 없습니다)'+(skipped?('\n권한 없는 '+skipped+'건은 제외됩니다.'):''))) return;
  Promise.all(mine.map(function(id){
    return fetch('/api/jp-press?id='+encodeURIComponent(id),{method:'DELETE',headers:authHeader()})
      .then(function(r){ return r.ok?id:null; }).catch(function(){ return null; });
  })).then(function(done){
    var okIds=done.filter(Boolean);
    pressItems=pressItems.filter(function(x){ return okIds.indexOf(x.id)<0; });
    okIds.forEach(function(id){ delete pressSel[id]; });
    if(pressView && okIds.indexOf(pressView)>=0) closePressView();
    renderPress();
    toast(okIds.length+'건 삭제됨'+(okIds.length<mine.length?(' · 실패 '+(mine.length-okIds.length)+'건'):''));
  });
}
...
function renderPressEditor(){
  ...
  b.innerHTML=
    ...
    '<label class="fl">단계</label><div class="seg statusseg" id="pe-status">'+
      PRESS_ST.map(function(d){ return '<button type="button" data-pst="'+d[0]+'"'+(pressStDef(pressEdit.status)[0]===d[0]?' class="on"':'')+'>'+esc(d[1])+'</button>'; }).join('')+
    '</div>'+
...
function init(){
  ...
  function newsDelegate(e){
    ...
    var en=e.target.closest('[data-news-en]'); if(en){ setNewsEn(en.getAttribute('data-news-en')); return; }
    var ne=e.target.closest('[data-nv-en]'); if(ne){ setNewsEn(ne.getAttribute('data-nv-en').split('~')[0], ne.getAttribute('data-nv-en').split('~')[1]); return; }
    var st=e.target.closest('[data-news-star]'); if(st){ var sp=st.getAttribute('data-news-star').split('~');
      var r=st.getBoundingClientRect(), half=(e.clientX-r.left) < r.width/2;
      setNewsPriority(sp[0], (+sp[1]) - (half?0.5:0)); return; }
    var nck=e.target.closest('[data-news-ck]'); if(nck){ newsSel[nck.getAttribute('data-news-ck')]=nck.checked; renderNews(); return; }
    if(e.target && e.target.id==='news-ckall'){ var non=e.target.checked; newsSel={};
      if(non) newsItems.filter(newsMatch).forEach(function(a){ newsSel[a.id]=true; });   // 걸러 놓은 것만 고른다
      renderNews(); return; }
    ...
    var nst=e.target.closest('[data-news-sort]'); if(nst){ var k=nst.getAttribute('data-news-sort');
      if(newsSort.key===k) newsSort.dir=(newsSort.dir==='asc'?'desc':'asc');
      else newsSort={key:k, dir:(k==='pri'||k==='date')?'desc':'asc'};
      renderNews(); return; }
    var nfc=e.target.closest('[data-news-nfclear]'); if(nfc){ newsFilter={stage:'',en:'',person:'',dept:'',q:''}; renderNews(); return; }
  ...
  if(nwl) nwl.addEventListener('change',function(e){ var f=e.target.closest('[data-nf]'); if(!f) return;
    newsFilter[f.getAttribute('data-nf')]=f.value; renderNews(); });
  if(nwl) nwl.addEventListener('input',function(e){ var f=e.target.closest('[data-nf="q"]'); if(!f) return;
    newsFilter.q=f.value; var at=document.activeElement===f; renderNews();
    if(at){ var n=document.querySelector('[data-nf="q"]'); if(n){ n.focus(); n.setSelectionRange(n.value.length,n.value.length); } } });
  var nwb=document.getElementById('news-bar'); if(nwb) nwb.addEventListener('click', newsDelegate);
  var nbtn=document.getElementById('noti-btn'); if(nbtn) nbtn.onclick=function(e){ e.stopPropagation(); toggleNoti(); if(notiOpen) loadNoti(); };
  // 탭을 다시 볼 때 · 5분마다 알림함을 확인한다(가벼운 GET 하나)
  document.addEventListener('visibilitychange',function(){ if(!document.hidden) loadNoti(); });
  setInterval(function(){ if(!document.hidden && Auth.authed()) loadNoti(); }, 300000);
  var nall=document.getElementById('noti-readall'); if(nall) nall.onclick=function(e){ e.stopPropagation(); notiMarkAll(); };
  var nlist=document.getElementById('noti-list');
  if(nlist) nlist.addEventListener('click',function(e){ var it=e.target.closest('[data-noti]'); if(!it) return;
    var id=it.getAttribute('data-noti'); notiMarkAll(); toggleNoti(false); setView('news'); openNewsView(id); });
  document.addEventListener('click',function(e){ if(notiOpen && !e.target.closest('.notiwrap')) toggleNoti(false); });
  ...
  if(nvb) nvb.addEventListener('input',function(e){ if(e.target.closest('[data-ac-input]')) growComment(e.target); });
  if(nvt) nvt.addEventListener('click', newsDelegate);
  if(nvb) nvb.addEventListener('keydown',function(e){
    if(e.key==='Enter'&&(e.metaKey||e.ctrlKey)&&!e.isComposing&&e.keyCode!==229){ var inp=e.target.closest('[data-ac-input]'); if(inp){ e.preventDefault(); addNewsComment(inp.getAttribute('data-ac-input')); } } });
  ...
  // ── 읽기 모달(게시판 상세) ──
  var pvx=document.getElementById('pv-close'); if(pvx) pvx.onclick=closePressView;
  var pvc=document.getElementById('pv-cancel'); if(pvc) pvc.onclick=closePressView;
  var pvs=document.getElementById('pressview-scrim');
  if(pvs) pvs.addEventListener('click',function(e){ if(e.target===this) closePressView(); });
  var pve=document.getElementById('pv-edit'); if(pve) pve.onclick=function(){ var id=pressView; closePressView(); if(id) openPressEditor(id); };
  var pvd=document.getElementById('pv-del'); if(pvd) pvd.onclick=function(){ if(pressView) deletePress(pressView); };
  var pvt=document.getElementById('pv-tools');
  if(pvt) pvt.addEventListener('click',function(e){ var b=e.target.closest('[data-pv-st]'); if(!b||!pressView) return;
    setPressStatus(pressView, b.getAttribute('data-pv-st')).then(function(){ renderPress(); renderPressView(); }); });
  ...
  // 목록(#press-list)과 일괄 작업 바(#press-bar) 둘 다에 같은 위임 핸들러를 건다.
  // 바에만 있는 버튼(일괄 단계·삭제·선택 해제)이 목록 리스너로는 잡히지 않는다.
  function pressDelegate(e){
    var vw=e.target.closest('[data-press-view]'); if(vw){ openPressView(vw.getAttribute('data-press-view')); return; }
    var bst=e.target.closest('[data-press-bulk-st]'); if(bst){ pressBulkStatus(bst.getAttribute('data-press-bulk-st')); return; }
    var bdl=e.target.closest('[data-press-bulk-del]'); if(bdl){ pressBulkDelete(); return; }
    var sn=e.target.closest('[data-press-selnone]'); if(sn){ pressSel={}; renderPress(); return; }
    var ck=e.target.closest('[data-press-ck]'); if(ck){ pressSel[ck.getAttribute('data-press-ck')]=ck.checked; renderPress(); return; }
    if(e.target && e.target.id==='press-ckall'){ var on=e.target.checked; pressSel={}; if(on) pressItems.forEach(function(a){ pressSel[a.id]=true; }); renderPress(); return; }
  ...
  var prl=document.getElementById('press-list'); if(prl) prl.addEventListener('click', pressDelegate);
  var prb=document.getElementById('press-bar'); if(prb) prb.addEventListener('click', pressDelegate);
  ...
  document.getElementById('news-body').addEventListener('click',function(e){
    var x=e.target.closest('[data-news-img-del]'); if(x&&newsEdit){ newsEdit.images.splice(+x.getAttribute('data-news-img-del'),1); renderNewsPhotos(); }
  });
  // 홍보부원 회원 모달 위임
  ...
  // 보드 열 너비 슬라이더 — CSS 변수만 갱신(재렌더 불필요)
  var bcw=document.getElementById('board-colw');
  if(bcw){ bcw.value=boardColW;
    bcw.addEventListener('input',function(){ var v=parseInt(this.value,10); if(isNaN(v)) return; boardColW=v; applyBoardColW(); try{localStorage.setItem('jamboree-plan:board-colw',String(v));}catch(_){} });
  }
  // 보드 열 표시(숨기기) 토글
  var bcols=document.getElementById('board-cols');
  if(bcols) bcols.addEventListener('click',function(e){ var b=e.target.closest('[data-colv]'); if(!b) return;
    var k=b.getAttribute('data-colv'); if(boardHidden[k]) delete boardHidden[k]; else boardHidden[k]=1; saveBoardHidden(); renderBoard(); });
