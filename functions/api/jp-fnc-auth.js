 ...
 *
 *  POST /api/jp-fnc-auth { id, pw }  → id/pw 가 foodservice/20260803 이면 서명 토큰 발급
 *      성공: { ok:true, token, exp }   실패: 200 { ok:false }
 *
 * ⚠️ 내부 운영 보드용 단순 게이트다(계정: foodservice/20260803, 2026-07-28 사용자 지정). 강한 보안이 아니라,
...
export async function onRequestPost({ request, env }) {
  ...
  if (id !== "foodservice" || pw !== "20260803") return json({ ok: false, error: "invalid" });
