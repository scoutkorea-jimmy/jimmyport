...
```bash
node test/regress-krjam-fnc.js      # 65 케이스
```
정적 자산만 쓰므로 **API 목업이 필요 없다**(서버·KV 무접촉). 커버: 33쪽 이미지·썸네일·PDF(33쪽)·OG 존재 및 200 · 넘김(버튼/키보드/해시/이미지 실제 교체) · 스크러버 · 목차(좌측 사이드바/모바일 드로어·챕터 9개·썸네일 16:9·활성·이동·접기·상태 저장) · 확대 · 딥링크(`#p12`·범위 밖) · 모바일(레이아웃·터치타깃·좌측 드로어·가로 보기 전환/배율/확대잠금) · 콘솔 에러 0.
