...
- **지금 현장**('오늘의 현장' 패널 안, 라이브): **진행 중** 일정과 **앞으로 3시간** 내 시작 일정을 **일정표 열(트랙) 단위**로 묶어 표시. 항목마다 `N분 남음`/`N분 뒤`, 클릭 시 해당 일정 열림. 분 단위 갱신. 일정표 보기 필터의 영향을 받지 않음.
...
- **트랙(열)**: `잼버리 일정 · 의전 · 홍보부 · 컵 1기 · 컵 2기 · 입·퇴영`. 일간 뷰에서 **존재하는 트랙만 별도 열**로 갈라지고, 열 사이 구분선을 끌어 너비 조절(더블클릭=기본값). 보기 필터 칩으로 트랙별 on/off.
- **입·퇴영 트랙**: 참가단 버스 호차의 입영(8/3~8/5)·퇴영(8/9)을 **한 열**에 두고 블록 태그(입영/퇴영)로 구분. 블록 30분 고정(원문에 시각만 있음), 방향은 `dir('in'|'out')`. 촬영 리스트에는 올라가지 않는다.
- **현재 시각 인디케이터**: 오늘 날짜 열에 빨간 가로선 + `HH:MM` 배지. 30초마다 선만 이동(그리드 재렌더 없음), 오늘이 8/2~8/9 밖이면 표시 안 함.
...
### 4.6-a 현장 위치 지도 (`sitemap`)
- **야영장 배치도**(2026-07-23 시설물자관리본부 최종본, 2000×1414) 위에 홍보부 인원을 **구역 단위**로 표시. 구역 31곳 — 본부·시설 19(JHQ·전시행사장·**대집회장**·**급식편의본부**·대회장야영장·**과정1~7**(과정6=체육관)·**안전본부**·물자보급·컵스숙소·**주차장**·관리사무소·잼버리병원·운영요원숙소) + 분단 영지 7 + 게이트 5.
- **지금 기준**(현재 시각 일정표로 실시간, 1분 갱신) ↔ **시간 지정**(날짜+슬라이더) 두 모드. 우선순위 = **수동 지정 > 일정표 > 기본(JHQ 본부)**. 아바타 드래그·클릭 이동, 수동 배치 시 **체크인 시각** 기록.
- **촬영 공백**: 기준 시각에 일정은 있는데 인원이 없는 구역을 앰버색으로 지도·목록에 표시. **촬영 요청**(제목·구역·시간·메모)은 카메라 핀으로.
- 일정/의전의 **장소명 자동 매칭**(`zoneForPlace`, 별칭 기반) + 일정 모달에서 구역 직접 지정. 같은 구역 목록을 `/krjam-jebo` 공개 제보 페이지가 **같은 key 로 공유**한다.
