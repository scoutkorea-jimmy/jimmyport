 ...
 * - 편집본은 localStorage 에 저장 → 공개 사이트(app.js)가 미리보기로 사용
...
(function () {
  ...
  function baseUnits() {
    return Array.isArray(window.SCOUT_UNITS) ? window.SCOUT_UNITS : [];
  }
  function clone(v) { return JSON.parse(JSON.stringify(v)); }

  // ── state ──────────────────────────────────────────────────────────
  var units;
  var map, unitLayer = L.layerGroup(), markers = [];
  var pickIndex = null;
  ...
  function uid() {
    return "unit-" + Date.now().toString(36) + Math.floor(Math.random() * 1e4).toString(36);
  }
  ...
  function saveDraft() {
    ...
    saveTimer = setTimeout(function () {
      try {
        ...
        setStatus("저장 실패: 저장 공간이 가득 찼을 수 있습니다.", false);
      }
    }, 300);
  }
  ...
  function selectOptions(values, current) {
    ...
    if (current && !has) {
      opts += '<option value="' + escAttr(current) + '" selected>' + escHtml(current) + " (사용자 입력)</option>";
    }
    return opts;
  }
  ...
  function field(label, name, value, type, span) {
    return (
      '<div class="field' + (span ? " col-span" : "") + '">' +
        "<label>" + label + "</label>" +
        '<input type="' + (type || "text") + '" data-field="' + name + '" value="' +
          escAttr(value) + '"' + (name === "lat" || name === "lng" ? ' step="any"' : "") +
          (name === "meetingDay" ? ' list="weekday-list"' : "") + " />" +
      "</div>"
    );
  }
  ...
  function cardHtml(u, i) {
    var secBoxes = SECTIONS.map(function (s) {
      ...
      return (
        '<label class="section-toggle' + (on ? " checked" : "") + '">' +
          '<input type="checkbox" data-section="' + escAttr(s) + '"' + (on ? " checked" : "") + " />" +
          escHtml(s) +
        "</label>"
      );
    }).join("");
    ...
    return (
      '<div class="editor-card" data-index="' + i + '">' +
        '<div class="editor-card-head">' +
          '<span class="editor-card-rank">' + (i + 1) + "</span>" +
          '<span class="editor-card-title">' + (escHtml(u.name) || "(이름 없음)") + "</span>" +
          '<span class="editor-card-actions">' +
            '<button type="button" class="icon-btn" data-action="up" title="위로" aria-label="위로 이동">↑</button>' +
            '<button type="button" class="icon-btn" data-action="down" title="아래로" aria-label="아래로 이동">↓</button>' +
        ...
        '<div class="field-grid">' +
          ...
          '<div class="field">' +
            "<label>종류 (학교대/지역대)</label>" +
            '<select data-field="type">' + selectOptions(TYPES, u.type) + "</select>" +
          "</div>" +

          '<div class="field">' +
            "<label>지방·특수연맹</label>" +
            '<select data-field="federation">' + selectOptions(FEDERATIONS, u.federation) + "</select>" +
          "</div>" +
          ...
          '<div class="field col-span">' +
            ...
            '<div class="coord-row">' +
              '<div class="field"><label class="sr-only">위도</label>' +
                '<input type="number" step="any" data-field="lat" value="' + escAttr(u.lat) + '" /></div>' +
              '<div class="field"><label class="sr-only">경도</label>' +
                '<input type="number" step="any" data-field="lng" value="' + escAttr(u.lng) + '" /></div>' +
              '<button type="button" class="admin-btn pick-btn" data-action="pick">지도에서 위치 지정</button>' +
            "</div>" +
          "</div>" +

          '<div class="field col-span"><label>비고</label>' +
            '<textarea data-field="note" rows="2">' + escHtml(u.note) + "</textarea></div>" +
        "</div>" +
      "</div>"
    );
  }
  ...
  function rebuildMarkers() {
    ...
    units.forEach(function (u, i) {
      ...
      var marker = L.marker([lat, lng], {
        icon: unitIcon(i + 1, i === activeIndex),
        draggable: true, title: u.name, alt: u.name,
      });
      marker.bindPopup(escHtml(u.name) + "<br>" + escHtml(u.federation) +
        (u.council ? " · " + escHtml(u.council) : ""));
  ...
  function setActive(i, pan) {
    activeIndex = i;
    var cards = $list.querySelectorAll(".editor-card");
    cards.forEach(function (c) {
      c.classList.toggle("is-active", Number(c.getAttribute("data-index")) === i);
    });
    markers.forEach(function (m, idx) {
      if (m) m.setIcon(unitIcon(idx + 1, idx === i));
    });
    var el = cardEl(i);
  ...
  function onMarkerDrag(i, latlng) {
    ...
    var el = cardEl(i);
    if (el) {
      el.querySelector('[data-field="lat"]').value = lat;
      el.querySelector('[data-field="lng"]').value = lng;
    }
    saveDraft();
  }
  ...
  function onFieldInput(e) {
    var input = e.target;
    var card = input.closest(".editor-card");
    if (!card) return;
    ...
    if (fieldName === "lat" || fieldName === "lng") {
      ...
      var m = markers[i];
      if (m && !isNaN(parseFloat(u.lat)) && !isNaN(parseFloat(u.lng))) {
        m.setLatLng([parseFloat(u.lat), parseFloat(u.lng)]);
      }
    } else {
      ...
      if (fieldName === "name") {
        ...
        if (markers[i]) markers[i].setPopupContent(
          escHtml(u.name) + "<br>" + escHtml(u.federation) + (u.council ? " · " + escHtml(u.council) : ""));
      }
    }
    saveDraft();
  }
  ...
  function onListClick(e) {
    ...
    if (btn) {
      ...
      if (action === "up") return doMove(i, -1);
      if (action === "down") return doMove(i, 1);
      if (action === "pick") return startPick(i);
      return;
    }
    var head = e.target.closest(".editor-card-head");
    if (head) {
      var idx = Number(head.closest(".editor-card").getAttribute("data-index"));
      setActive(idx, true);
    }
  }
  ...
  function doAdd() {
    ...
    units.push({
      id: uid(), name: "새 단위대", type: "지역대",
      federation: "", council: "", address: "",
      lat: round5(center.lat), lng: round5(center.lng),
      sections: [], meetingDay: "", contact: "", note: "",
    });
    ...
    if (markers[activeIndex]) markers[activeIndex].openPopup();
  }
  ...
  function startPick(i) {
    ...
    $hint.textContent = (i + 1) + "번 \"" + (units[i].name || "단위대") + "\" — 지도를 클릭해 위치를 지정하세요. (취소: ESC)";
    setActive(i, true);
  }
  ...
  function onMapClick(e) {
    ...
    var i = pickIndex;
    onMarkerDrag(i, e.latlng);          // 좌표 + 입력칸 반영 + 저장
    if (markers[i]) markers[i].setLatLng(e.latlng);
    else { rebuildMarkers(); }
    stopPick();
  }
  ...
  function dataJsText() {
    return (
      "// ⬇️ 실제 데이터로 교체\n" +
      "// scout-finder 데이터. 이 배열만 교체하면 app.js 수정 없이 동작합니다.\n" +
  ...
  function doCopyJson() {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text)
        .then(function () { setStatus("JSON 복사됨", true); })
  ...
  function fallbackCopy(text) {
    ...
    catch (e) { setStatus("복사 실패 — 콘솔에서 확인하세요", false); console.log(text); }
  ...
  function doImport(file) {
    ...
    reader.onload = function () {
      try {
        ...
        alert("가져오기 실패: 올바른 JSON 배열이 아닙니다.\n" + e.message);
      }
    };
  ...
  function doReset() {
    ...
    units = clone(baseUnits());
    activeIndex = null;
    render();
    setStatus("샘플로 초기화됨", true);
  }

  // ── init ───────────────────────────────────────────────────────────
  function init() {
    // 초기 데이터: localStorage 편집본 우선
    var draft = null;
    try {
      var raw = localStorage.getItem(DRAFT_KEY);
      if (raw) { var a = JSON.parse(raw); if (Array.isArray(a)) draft = a; }
    } catch (e) { /* noop */ }
    ...
    document.addEventListener("keydown", function (e) {
      if (e.key === "Escape" && pickIndex !== null) stopPick();
    });
  }
